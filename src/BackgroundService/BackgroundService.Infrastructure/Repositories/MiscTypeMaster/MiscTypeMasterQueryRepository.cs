...
namespace BackgroundService.Infrastructure.Repositories.MiscTypeMaster
{
    public class MiscTypeMasterQueryRepository : IMiscTypeMasterQueryRepository
    {
        ...
        public async Task<(List<Domain.Entities.Notification.MiscTypeMaster>, int)> GetAllMiscTypeMasterAsync(int PageNumber, int PageSize, string? SearchTerm)
        {
            var query = $$"""
            ...
            var misctypemaster = (await misctype.ReadAsync<Domain.Entities.Notification.MiscTypeMaster>()).ToList();
            int totalCount = (await misctype.ReadFirstAsync<int>());
            return (misctypemaster, totalCount);
        }
        public async Task<Domain.Entities.Notification.MiscTypeMaster?> GetByMiscTypeMasterCodeAsync(string name, int? id = null)
        {
            var query = """
            ...
            return await _dbConnection.QueryFirstOrDefaultAsync<Domain.Entities.Notification.MiscTypeMaster>(query, parameters);
        }

        public async Task<Domain.Entities.Notification.MiscTypeMaster> GetByIdAsync(int id)
        {
            const string query = @" SELECT Id,MiscTypeCode,Description,IsActive  FROM appdata.MiscTypeMaster          
             WHERE Id = @id AND IsDeleted = 0";
            return await _dbConnection.QueryFirstOrDefaultAsync<Domain.Entities.Notification.MiscTypeMaster>(query, new { id });
        }
        public async Task<List<Domain.Entities.Notification.MiscTypeMaster>> GetMiscTypeMaster(string searchPattern)
        {
            ...
            var misctypemaster = await _dbConnection.QueryAsync<Domain.Entities.Notification.MiscTypeMaster>(query, parameters);
