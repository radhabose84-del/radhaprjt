    ...
    public class LotMasterQueryRepository : ILotMasterQueryRepository
    {
            ...
            int pageNumber, int pageSize, string? searchTerm)
        {
            ...
            var query = $@"
                ...
                LEFT JOIN Sales.MiscMaster lt ON lm.LotTypeId = lt.Id AND lt.IsDeleted = 0
                LEFT JOIN Sales.MiscMaster st ON lm.StatusId  = st.Id AND st.IsDeleted = 0
        ...
        {
            const string sql = @"
                SELECT COUNT(1) FROM Sales.LotMaster
