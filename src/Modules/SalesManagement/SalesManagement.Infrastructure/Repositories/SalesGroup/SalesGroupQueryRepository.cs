        ...
        public async Task<bool> SoftDeleteValidationAsync(int id)
        {
            const string sql = @"
                SELECT CASE WHEN EXISTS (
                    SELECT 1 FROM [Sales].[OfficerSalesGroup] WHERE SalesGroupId = @id AND IsDeleted = 0
                    UNION ALL
                    SELECT 1 FROM [Sales].[DiscountSalesGroup] WHERE SalesGroupId = @id AND IsDeleted = 0
                    UNION ALL
                    SELECT 1 FROM [Sales].[AgentCommissionSalesGroup] WHERE SalesGroupId = @id AND IsDeleted = 0
                    UNION ALL
                    SELECT 1 FROM [Sales].[SalesOrderHeader] WHERE SalesGroupId = @id AND IsDeleted = 0
                ) THEN 1 ELSE 0 END";

            return await _dbConnection.ExecuteScalarAsync<bool>(sql, new { id });
        }

        public async Task<bool> IsSalesGroupLinkedAsync(int id)
        {
            const string sql = @"
                SELECT CASE WHEN EXISTS (
                    SELECT 1 FROM [Sales].[OfficerSalesGroup] WHERE SalesGroupId = @id AND IsDeleted = 0 AND IsActive = 1
                    UNION ALL
                    SELECT 1 FROM [Sales].[DiscountSalesGroup] WHERE SalesGroupId = @id AND IsDeleted = 0 AND IsActive = 1
                    UNION ALL
                    SELECT 1 FROM [Sales].[AgentCommissionSalesGroup] WHERE SalesGroupId = @id AND IsDeleted = 0 AND IsActive = 1
                    UNION ALL
                    SELECT 1 FROM [Sales].[SalesOrderHeader] WHERE SalesGroupId = @id AND IsDeleted = 0 AND IsActive = 1
                ) THEN 1 ELSE 0 END";
