...
{
    public class GetPendingSalesOrderAmendmentByIdQueryHandler
        : IRequestHandler<GetPendingSalesOrderAmendmentByIdQuery, SalesOrderAmendmentHeaderDto?>
    {
        private readonly ISalesOrderAmendmentQueryRepository _repo;
        private readonly IWorkflowLookup _workflowLookup;
        private readonly IUserLookup _userLookup;
        private readonly IIPAddressService _ipAddressService;
        private readonly IMediator _mediator;

        public GetPendingSalesOrderAmendmentByIdQueryHandler(
            ISalesOrderAmendmentQueryRepository repo,
        ...
        public async Task<SalesOrderAmendmentHeaderDto?> Handle(
            GetPendingSalesOrderAmendmentByIdQuery request, CancellationToken ct)
        {
            ...
            var wfApprovers = await _workflowLookup
                .GetApproverListAsync(MiscEnumEntity.TransactionTypeSalesOrderAmendment, [result.Id]);
            ...
            // Attach workflow fields to DTO via extended properties on the DTO
            // (reuse ApprovedBy / ApprovedDate as a carrier — the DTO has ApprovedBy)
            // We add a separate PendingSalesOrderAmendmentByIdDto wrapper
            result.ApprovedBy = approverId;

            await _mediator.Publish(new AuditLogsDomainEvent(
                actionDetail: "GetPendingById",
                actionCode: "GetPendingSalesOrderAmendmentByIdQuery",
                actionName: result.Id.ToString(),
                details: $"Sales Order Amendment pending Id {result.Id} fetched.",
                module: "SalesOrderAmendment"), ct);
