...
{
    public class GetPendingSalesOrderAmendmentQueryHandler
        : IRequestHandler<GetPendingSalesOrderAmendmentQuery, (List<PendingSalesOrderAmendmentDto> Items, int TotalCount)>
    {
        ...
        public GetPendingSalesOrderAmendmentQueryHandler(
        ...
        public async Task<(List<PendingSalesOrderAmendmentDto> Items, int TotalCount)> Handle(
            GetPendingSalesOrderAmendmentQuery request, CancellationToken ct)
        {
            ...
            var wfApprovers = await _workflowLookup
                .GetApproverListAsync(MiscEnumEntity.TransactionTypeSalesOrderAmendment, moduleIds);
        ...
        private Task PublishAudit(int count, GetPendingSalesOrderAmendmentQuery q, CancellationToken ct)
            => _mediator.Publish(new AuditLogsDomainEvent(
                actionDetail: "GetAll-Pending",
                actionCode: string.Empty,
                actionName: "SalesOrderAmendmentPending",
