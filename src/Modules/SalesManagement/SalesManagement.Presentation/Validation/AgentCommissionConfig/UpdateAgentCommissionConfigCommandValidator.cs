            ...
            foreach (var rule in _validationRules)
            {
                switch (rule.Rule)
                {
                    case "NotEmpty":
                        ...
                        RuleFor(x => x.ItemId)
                            .GreaterThan(0)
                            .WithMessage($"{nameof(UpdateAgentCommissionConfigCommand.ItemId)} {rule.Error}");
                        ...
                        RuleFor(x => x.SubAgentPercentage)
                            .GreaterThanOrEqualTo(0)
                            .WithMessage($"{nameof(UpdateAgentCommissionConfigCommand.SubAgentPercentage)} must be greater than or equal to 0.")
                            .When(x => x.SubAgentPercentage.HasValue);

                        RuleFor(x => x.SubAgentPercentage)
                            .LessThanOrEqualTo(x => x.CommissionPercentage)
                            .WithMessage($"{nameof(UpdateAgentCommissionConfigCommand.SubAgentPercentage)} must be less than or equal to CommissionPercentage.")
                            .When(x => x.SubAgentPercentage.HasValue);
                        break;
                    ...
                    case "FKColumnDelete":
                        ...
                        RuleFor(x => x.UomId!.Value)
                            ...
                            .WithMessage($"{nameof(UpdateAgentCommissionConfigCommand.UomId)} {rule.Error}")
                    ...
                    case "AlreadyExists":
                        ...
                        RuleFor(x => x)
                            ...
                            .When(x => x.Id > 0 && x.AgentId > 0 && x.SalesSegmentId > 0 && x.ItemId > 0
