...
{
    public class DeleteItemPriceMasterCommandValidator : AbstractValidator<DeleteItemPriceMasterCommand>
    {
        private readonly List<ValidationRule> _validationRules;
        private readonly IItemPriceMasterQueryRepository _queryRepository;

        public DeleteItemPriceMasterCommandValidator(IItemPriceMasterQueryRepository queryRepository)
        {
            ...
            foreach (var rule in _validationRules)
            {
                switch (rule.Rule)
                {
                    case "NotEmpty":
                        RuleFor(x => x.Id)
                            .NotEmpty()
                            .WithMessage($"{nameof(DeleteItemPriceMasterCommand.Id)} {rule.Error}");
                        break;
