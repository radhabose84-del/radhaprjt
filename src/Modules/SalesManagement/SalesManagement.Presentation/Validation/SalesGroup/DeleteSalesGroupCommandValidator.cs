...
{
    public class DeleteSalesGroupCommandValidator : AbstractValidator<DeleteSalesGroupCommand>
    {
        private readonly List<ValidationRule> _validationRules;
        private readonly ISalesGroupQueryRepository _queryRepository;

        public DeleteSalesGroupCommandValidator(ISalesGroupQueryRepository queryRepository)
        {
            ...
            foreach (var rule in _validationRules)
            {
                switch (rule.Rule)
                {
                    case "NotEmpty":
                        RuleFor(x => x.Id)
                            .NotEmpty()
                            .WithMessage($"{nameof(DeleteSalesGroupCommand.Id)} {rule.Error}");
                        break;
