...
namespace FinanceManagement.Infrastructure.Repositories.MiscMaster
{
    public class MiscMasterQueryRepository : IMiscMasterQueryRepository
    {
        ...
        // No Finance.* dependents reference MiscMaster yet — extend with EXISTS clauses when added.
        public Task<bool> SoftDeleteValidationAsync(int id) => Task.FromResult(false);

        public Task<bool> IsMiscMasterLinkedAsync(int id) => Task.FromResult(false);

        public async Task<FinanceManagement.Domain.Entities.MiscMaster?> GetMiscMasterByName(string miscTypeCode, string miscTypeName)
        {
            const string sql = @"
                SELECT M.Id, M.Code, M.Description
                FROM Finance.MiscMaster AS M
                INNER JOIN Finance.MiscTypeMaster AS MT ON MT.Id = M.MiscTypeId
                WHERE M.IsDeleted = 0 AND M.IsActive = 1
                AND MT.IsDeleted = 0
                AND LOWER(MT.MiscTypeCode) = LOWER(@MiscTypeCode)
                AND LOWER(M.Code) = LOWER(@MiscTypeName);";

            return await _dbConnection.QueryFirstOrDefaultAsync<FinanceManagement.Domain.Entities.MiscMaster>(sql, new { MiscTypeCode = miscTypeCode, MiscTypeName = miscTypeName });
        }

        public async Task<FinanceManagement.Domain.Entities.MiscMaster?> GetMiscMasterByCode(string code)
        {
            const string sql = @"
                SELECT M.Id, M.Code, M.Description
                FROM Finance.MiscMaster AS M
            ...
            return await _dbConnection.QueryFirstOrDefaultAsync<FinanceManagement.Domain.Entities.MiscMaster>(sql, new { Code = code });
        }
    }
}
