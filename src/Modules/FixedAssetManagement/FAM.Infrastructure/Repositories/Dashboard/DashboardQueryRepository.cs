...
namespace FAM.Infrastructure.Repositories.Dashboard
{
    public class DashboardQueryRepository : IDashboardQueryRepository
    {
        ...
        public async Task<ChartDto> GetAssetChartViewAsync(int? departmentId, DateTime fromDate, DateTime toDate)
        {
            ...
            var query = @"
                ...
                WHERE
                    ...
                    AND ap.PoDate BETWEEN @FromDate AND @ToDate
                GROUP BY ag.GroupName
                ORDER BY ag.GroupName;
            ";

            var result = await _dbConnection.QueryAsync<AssetGroupSummaryDto>(query, new { UnitId = unitId, DepartmentId = departmentId, FromDate = fromDate, ToDate = toDate });
        ...
        public async Task<ChartDto> GetAssetExpiredDashBoardDataAsync(DateTime fromDate, DateTime toDate)
        {
            var unitId = _iPAddressService.GetUnitId() ?? 0;
            ...
            var query = @"
                ...
                WHERE
                    A.UnitId = @UnitId
                    AND DATEADD(YEAR, D.UsefulLife, B.PoDate) BETWEEN @FromDate AND @ToDate
                GROUP BY
            ...
            var result = await _dbConnection.QueryAsync<AssetExpiredDashBoardDto>(query, new { UnitId = unitId, FromDate = fromDate, ToDate = toDate });
        ...
        public async Task<CardViewDto> GetDashboardDataAsync(DateTime fromDate, DateTime toDate)
        {
            var UnitId = _iPAddressService.GetUnitId() ?? 0;
            ...
            var query = @"
                SELECT
                    COUNT(*) AS TotalAssets,
                    SUM(ISNULL(ap.PurchaseValue, 0)) AS TotalAssetValue,
                    SUM(CASE WHEN am.CreatedDate BETWEEN @FromDate AND @ToDate THEN 1 ELSE 0 END) AS NewAssets,
                    SUM(CASE WHEN am.CreatedDate BETWEEN @FromDate AND @ToDate THEN ISNULL(ap.PurchaseValue, 0) ELSE 0 END) AS NewAssetsValue,
                    (
                        SELECT COUNT(*)
                        FROM FixedAsset.AssetDisposal d
                        INNER JOIN FixedAsset.AssetMaster am2 ON d.AssetId = am2.Id
            ...
            var result = await _dbConnection.QueryFirstOrDefaultAsync<CardViewDto>(query, new { UnitId, FromDate = fromDate, ToDate = toDate });
