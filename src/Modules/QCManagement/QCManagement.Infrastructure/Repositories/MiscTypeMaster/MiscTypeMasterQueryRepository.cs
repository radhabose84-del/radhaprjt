...
namespace QCManagement.Infrastructure.Repositories.MiscTypeMaster
{
    public class MiscTypeMasterQueryRepository : IMiscTypeMasterQueryRepository
    {
        ...
        public async Task<bool> SoftDeleteValidationAsync(int id)
        {
            const string sql = @"
                SELECT CASE WHEN EXISTS (
                    SELECT 1 FROM [QC].[MiscMaster] WHERE MiscTypeId = @id AND IsDeleted = 0
                ) THEN 1 ELSE 0 END";
        ...
        public async Task<bool> IsMiscTypeMasterLinkedAsync(int id)
        {
            const string sql = @"
                SELECT CASE WHEN EXISTS (
                    SELECT 1 FROM [QC].[MiscMaster] WHERE MiscTypeId = @id AND IsDeleted = 0 AND IsActive = 1
                ) THEN 1 ELSE 0 END";
