...
namespace GateEntryManagement.Infrastructure.Repositories.MiscMaster
{
    public class MiscMasterQueryRepository : IMiscMasterQueryRepository
    {
        ...
        public async Task<(List<MiscMasterDto>, int)> GetAllAsync(int pageNumber, int pageSize, string? searchTerm, int? miscTypeId = null)
        {
            ...
            var query = $@"
                ...
                FROM Gate.MiscMaster mm
                INNER JOIN Gate.MiscTypeMaster mtm ON mm.MiscTypeId = mtm.Id AND mtm.IsDeleted = 0
                WHERE {whereClause}
        ...
        public async Task<GateEntryManagement.Domain.Entities.MiscMaster?> GetMiscMasterByName(string miscTypeCode, string miscTypeName)
        {
            const string sql = @"
                SELECT M.Id, M.Code, M.Description
                FROM Gate.MiscMaster AS M
                INNER JOIN Gate.MiscTypeMaster AS MT ON MT.Id = M.MiscTypeId
                WHERE M.IsDeleted = 0 AND M.IsActive = 1
                AND MT.IsDeleted = 0
                AND LOWER(MT.MiscTypeCode) = LOWER(@MiscTypeCode)
                AND LOWER(M.Code) = LOWER(@MiscTypeName);";

            return await _dbConnection.QueryFirstOrDefaultAsync<GateEntryManagement.Domain.Entities.MiscMaster>(sql, new { MiscTypeCode = miscTypeCode, MiscTypeName = miscTypeName });
        }
        ...
        public async Task<Domain.Entities.MiscMaster?> GetMiscMasterByCode(string code)
        {
            ...
            return await _dbConnection.QueryFirstOrDefaultAsync<Domain.Entities.MiscMaster>(sql, new { Code = code });
        }
    }
}
