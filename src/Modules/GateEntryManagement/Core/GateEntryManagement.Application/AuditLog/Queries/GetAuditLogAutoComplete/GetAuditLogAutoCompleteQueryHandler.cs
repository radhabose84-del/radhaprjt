...
namespace GateEntryManagement.Application.AuditLog.Queries.GetAuditLogBySearchPattern
{
    public class GetAuditLogBySearchPatternQueryHandler : IRequestHandler<GetAuditLogBySearchPatternQuery, ApiResponseDTO<List<AuditLogDto>>>
    {
        ...
        public async Task<ApiResponseDTO<List<AuditLogDto>>> Handle(GetAuditLogBySearchPatternQuery request, CancellationToken cancellationToken)
        {
            var filter = Builders<AuditLogs>.Filter.Or(
                Builders<AuditLogs>.Filter.Regex("CreatedByName", new BsonRegularExpression(request.SearchPattern, "i")),
                Builders<AuditLogs>.Filter.Regex("Action", new BsonRegularExpression(request.SearchPattern, "i")),
                Builders<AuditLogs>.Filter.Regex("Details", new BsonRegularExpression(request.SearchPattern, "i"))
            );
