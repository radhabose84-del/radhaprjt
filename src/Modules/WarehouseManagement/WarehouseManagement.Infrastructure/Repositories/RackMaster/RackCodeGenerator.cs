...
namespace WarehouseManagement.Infrastructure.Repositories.RackMaster
{
    public class RackCodeGenerator : IRackCodeGenerator
    {
        ...
        public RackCodeGenerator(ApplicationDbContext dbContext, IMiscMasterLookup miscMasterLookup)
        {
        ...
        public async Task<string> GenerateAsync(int warehouseId, int? floorId, int? aisleId, int? rackLevelId)
        {
            ...
            {
                var wtList = await _miscMasterLookup.GetMiscMasterByIdAsync("WarehouseType");
        ...
        {
            var items = await _miscMasterLookup.GetMiscMasterByIdAsync(miscType);
