...
namespace MaintenanceManagement.Infrastructure.Repositories.MiscTypeMaster
{
    ...
    public MiscTypeMasterQueryRepository(IDbConnection dbConnection)
          ...
          public async Task<(List<MaintenanceManagement.Domain.Entities.MiscTypeMaster>,int)> GetAllMiscTypeMasterAsync(int PageNumber, int PageSize, string SearchTerm)
        {
                 var query = $$"""
             ...
             var misctypemaster = (await misctype.ReadAsync<MaintenanceManagement.Domain.Entities.MiscTypeMaster>()).ToList();
             int totalCount = (await misctype.ReadFirstAsync<int>());
            return (misctypemaster, totalCount);
        }
          public async Task<MaintenanceManagement.Domain.Entities.MiscTypeMaster> GetByMiscTypeMasterCodeAsync(string name, int? id = null)
        {
              var query = """
            ...
            return await _dbConnection.QueryFirstOrDefaultAsync<MaintenanceManagement.Domain.Entities.MiscTypeMaster>(query, parameters);
        } 

            public async Task<MaintenanceManagement.Domain.Entities.MiscTypeMaster> GetByIdAsync(int id)
        {            
           const string query = @" SELECT Id,MiscTypeCode,Description,IsActive  FROM Maintenance.MiscTypeMaster          
             WHERE Id = @id AND IsDeleted = 0";                          
            return await _dbConnection.QueryFirstOrDefaultAsync<MaintenanceManagement.Domain.Entities.MiscTypeMaster>(query, new { id });
        }
          public async Task<List<MaintenanceManagement.Domain.Entities.MiscTypeMaster>>  GetMiscTypeMaster(string searchPattern)
        {
            ...
            var misctypemaster = await _dbConnection.QueryAsync<MaintenanceManagement.Domain.Entities.MiscTypeMaster>(query, parameters);
