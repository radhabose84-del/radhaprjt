...
namespace MaintenanceManagement.Application.PreventiveSchedulers.Commands.MachineWiseFrequencyUpdate
{
    public class MachineWiseFrequencyUpdateCommandHandler : IRequestHandler<MachineWiseFrequencyUpdateCommand, ApiResponseDTO<bool>>
    {
        ...
        public MachineWiseFrequencyUpdateCommandHandler(IPreventiveSchedulerCommand preventiveSchedulerCommand,
            IPreventiveSchedulerQuery preventiveSchedulerQuery, IBackgroundServiceClient backgroundServiceClient,
            IMiscMasterQueryRepository miscMasterQueryRepository, IPreventiveScheduleLogService preventiveScheduleLogService,
        ...
        public async Task<ApiResponseDTO<bool>> Handle(MachineWiseFrequencyUpdateCommand request, CancellationToken cancellationToken)
        {
            ...
            if (request.IsActive == 1)
            {
                var (nextDate, reminderDate) = await _preventiveSchedulerQuery.CalculateNextScheduleDate(
                    request.LastMaintenanceActivityDate.ToDateTime(TimeOnly.MinValue),
                    request.FrequencyInterval, frequencyUnit.Code ?? "", DetailResult.ReminderWorkOrderDays);
                var (ItemNextDate, ItemReminderDate) = await _preventiveSchedulerQuery.CalculateNextScheduleDate(
                    request.LastMaintenanceActivityDate.ToDateTime(TimeOnly.MinValue),
            ...
            await _preventiveSchedulerCommand.UpdateScheduleDetails(DetailResult);
