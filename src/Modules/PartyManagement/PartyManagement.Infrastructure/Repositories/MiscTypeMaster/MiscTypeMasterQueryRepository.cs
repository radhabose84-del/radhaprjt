...
namespace PartyManagement.Infrastructure.Repositories.MiscTypeMaster
{
    public class MiscTypeMasterQueryRepository : IMiscTypeMasterQueryRepository
    {
            ...
            public async Task<(List<PartyManagement.Domain.Entities.MiscTypeMaster>,int)> GetAllMiscTypeMasterAsync(int PageNumber, int PageSize, string SearchTerm)
        {
             ...
             var misctypemaster = (await misctype.ReadAsync<PartyManagement.Domain.Entities.MiscTypeMaster>()).ToList();
          ...
          public async Task<PartyManagement.Domain.Entities.MiscTypeMaster> GetByMiscTypeMasterCodeAsync(string name, int? id = null)
        {
            ...
            return await _dbConnection.QueryFirstOrDefaultAsync<PartyManagement.Domain.Entities.MiscTypeMaster>(query, parameters);
        } 

            public async Task<PartyManagement.Domain.Entities.MiscTypeMaster> GetByIdAsync(int id)
        {            
           const string query = @" SELECT Id,MiscTypeCode,Description,IsActive  FROM Party.MiscTypeMaster          
             WHERE Id = @id AND IsDeleted = 0";                          
            return await _dbConnection.QueryFirstOrDefaultAsync<PartyManagement.Domain.Entities.MiscTypeMaster>(query, new { id });
        }
          public async Task<List<PartyManagement.Domain.Entities.MiscTypeMaster>>  GetMiscTypeMaster(string searchPattern)
        {
            

            const string query = @"SELECT Id, MiscTypeCode   FROM Party.MiscTypeMaster
            ...
            var misctypemaster = await _dbConnection.QueryAsync<PartyManagement.Domain.Entities.MiscTypeMaster>(query, parameters);
            return misctypemaster.ToList();
        }
          public async Task<bool> AlreadyExistsAsync(string miscTypeCode, int? id = null)
        {   

              var query = "SELECT COUNT(1) FROM Party.MiscTypeMaster WHERE MiscTypeCode = @miscTypeCode AND IsDeleted = 0";
        ...
        {
             var query = "SELECT COUNT(1) FROM  Party.MiscTypeMaster WHERE Id = @Id AND IsDeleted = 0";
