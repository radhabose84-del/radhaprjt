...
{
    public class UploadGrnEntryDocumentCommandHandler : IRequestHandler<UploadGrnEntryDocumentCommand, GRNReceivedImageDto>
    {
        private readonly IGRNEntryQueryRepository _iGrnEntryQueryRepository;

        public UploadGrnEntryDocumentCommandHandler(IGRNEntryQueryRepository iGrnEntryQueryRepository)
        {
            _iGrnEntryQueryRepository = iGrnEntryQueryRepository;
        }

        public async Task<GRNReceivedImageDto> Handle(UploadGrnEntryDocumentCommand request, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                var response = new GRNReceivedImageDto
                {
                    ImagePath = formattedPath,  // ✅ Correctly formatted file path
                    GrnEntryDocumentBase64 = base64Image  // ✅ Convert to Base64
                };
                return  response;
            }
            catch (Exception ex)
