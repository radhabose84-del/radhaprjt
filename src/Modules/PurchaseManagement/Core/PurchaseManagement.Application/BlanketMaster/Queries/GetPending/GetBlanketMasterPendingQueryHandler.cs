...
public sealed class GetBlanketMasterPendingQueryHandler
    : IRequestHandler<GetBlanketMasterPendingQuery, (List<GetBlanketMasterPendingGroupDto> Items, int TotalCount)>
{
    ...
    public GetBlanketMasterPendingQueryHandler(
    ...
    public async Task<(List<GetBlanketMasterPendingGroupDto> Items, int TotalCount)> Handle(
        GetBlanketMasterPendingQuery request, CancellationToken ct)
    {
        var (rows, _) = await _repo.GetBlanketMasterPendingAsync(
        ...
        var blanketIds = rows.Select(r => r.Id)
                             .Where(id => id > 0)
                             .Distinct()
                             .ToList();

        var wfApprovers = await _workflowLookup
            .GetApproverListAsync(MiscEnumEntity.TransactionTypeBlanket, blanketIds);
        ...
        foreach (var g in rows)
            g.Lines ??= new List<GetBlanketMasterPendingDto>();
