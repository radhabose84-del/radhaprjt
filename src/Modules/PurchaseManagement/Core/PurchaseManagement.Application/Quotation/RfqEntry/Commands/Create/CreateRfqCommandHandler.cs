...
//             IUOMGrpcClient uomGrpcClient, ITimeZoneService timeZoneService, IPurchaseIndentCommand indentRepo)
//         {
...
//             _itemGrpcClient = itemGrpcClient ?? throw new ArgumentNullException(nameof(itemGrpcClient));
//             _uomGrpcClient = uomGrpcClient ?? throw new ArgumentNullException(nameof(uomGrpcClient));
...
//                 if (uomIds.Count > 0)
//                 {
//                     var allUoms = await _uomGrpcClient.GetUOMAsync() ?? new List<Contracts.Dtos.Inventory.UOMMasterDto>();
//                     uomMap = allUoms
...
//                 if (itemIds.Count > 0)
//                 {
//                     var lite = await _itemGrpcClient.GetItemsByIdsAsync(itemIds, ct) ?? new List<Contracts.Dtos.Inventory.ItemMasterDto>();
//                     itemMap = lite
//                         .GroupBy(i => i.Id)
//                         .ToDictionary(g => g.Key, g =>
//                         {
