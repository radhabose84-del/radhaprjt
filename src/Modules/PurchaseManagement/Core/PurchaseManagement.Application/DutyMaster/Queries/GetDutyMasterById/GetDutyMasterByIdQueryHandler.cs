...
//         {
//             var hsn = await FetchHsnByCodeAsync(hsnGrpc, dto.HsnCode!, ct);
//             if (hsn is not null)
//             {
//                 dto.Hsn = new HsnInfo
//                 {
...
//                     Gst = hsn.GSTPercentage.ToString(),
//                     IGst = hsn.IGSTPercentage.ToString()
//                 };
//             }
//         }
...
//     private static async Task<HSNMasterDto?> FetchHsnByCodeAsync(IHSNGrpcClient hsnGrpc, string code, CancellationToken ct)
//     {
//         // Your client only supports GetAllAsync(page,size,search).
//         var (list, _) = await hsnGrpc.GetAllAsync(1, 1, code);
