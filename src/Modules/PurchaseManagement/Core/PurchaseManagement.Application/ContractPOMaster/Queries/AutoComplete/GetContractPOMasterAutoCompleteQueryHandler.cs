...
public sealed class GetContractPOMasterAutoCompleteQueryHandler
    : IRequestHandler<GetContractPOMasterAutoCompleteQuery, IReadOnlyList<ContractPOLookupDto>>
{
    private readonly IContractPOMasterQueryRepository _repo;
    private readonly IMediator _mediator;
    private readonly IPartyLookup _partyLookup;

    public GetContractPOMasterAutoCompleteQueryHandler(
    ...
    {
        var results = await _repo.AutocompleteAsync(request.Term ?? string.Empty, request.ApprovedOnly, request.VendorId, request.PODate ?? DateTimeOffset.UtcNow, ct);
