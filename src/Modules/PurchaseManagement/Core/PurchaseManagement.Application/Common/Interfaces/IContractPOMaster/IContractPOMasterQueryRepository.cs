...
public interface IContractPOMasterQueryRepository
{
    ...
    Task<IReadOnlyList<ContractPOLookupDto>> AutocompleteAsync(string term, bool approvedOnly, int? vendorId, DateTimeOffset? poDate, CancellationToken ct);
    ...
    Task<(List<GetContractPOMasterPendingGroupDto> Items, int Total)> GetContractPOMasterPendingAsync(
        int page, int size, string? search, CancellationToken ct);
    Task<bool> HasOverlappingContractAsync(int vendorId, DateTimeOffset validityFrom, DateTimeOffset validityTo, int? excludeId, CancellationToken ct);
}
