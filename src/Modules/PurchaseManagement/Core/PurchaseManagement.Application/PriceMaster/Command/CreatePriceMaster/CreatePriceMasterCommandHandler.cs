...
//         public CreatePriceMasterCommandHandler(IPriceMasterCommandRepository repo, IMapper mapper, IIPAddressService ipAddress, IMiscMasterQueryRepository miscRepo, IEventPublisher eventPublisher, IMediator mediator, ILogger<CreatePriceMasterCommandHandler> logger, IItemGrpcClient itemGrpc, IPartyGrpcClient partyGrpc)
//         {
//             _repo = repo;
//             _mapper = mapper;
...
//             try
//             {
//                 var itemTask   = _itemGrpc.GetItemsByIdsAsync(new[] { header.ItemId }, ct);
//                 var partyTask  = _partyGrpc.GetPartyByIdAsync(header.VendorId);

//                 await Task.WhenAll(itemTask, partyTask);

//                 var itemDto   = (await itemTask).FirstOrDefault();
//                 var partyDto  = await partyTask;
