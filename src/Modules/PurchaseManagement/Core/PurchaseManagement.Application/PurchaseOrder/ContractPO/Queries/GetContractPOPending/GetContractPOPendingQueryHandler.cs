...
{
    public sealed class GetContractPOPendingQueryHandler
        : IRequestHandler<GetContractPOPendingQuery, (List<GetContractPOPendingGroupDto> Items, int TotalCount)>
    {
        ...
        public GetContractPOPendingQueryHandler(
        ...
        public async Task<(List<GetContractPOPendingGroupDto> Items, int TotalCount)> Handle(
            GetContractPOPendingQuery request, CancellationToken ct)
        {
            var (rows, _) = await _repo.GetContractPOPendingAsync(
                request.PageNumber ?? 1, request.PageSize ?? 15, request.SearchTerm, request.PoId, ct);
            ...
            var wfApprovers = await _workflowLookup
                .GetApproverListAsync(MiscEnumEntity.TransactionTypeCPO.ToString(), poIds);
            ...
            foreach (var g in rows)
                g.Lines ??= new List<GetContractPOPendingDto>();
        ...
        private Task PublishAudit(int count, GetContractPOPendingQuery q, CancellationToken ct)
            => _mediator.Publish(new AuditLogsDomainEvent(
                actionDetail: "GetAll-Pending",
                actionCode: string.Empty,
                actionName: "ContractPOPending",
