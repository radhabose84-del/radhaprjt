...
namespace ProductionManagement.Infrastructure.Data.Configurations
{
    public class ProductionPackDetailConfiguration : IEntityTypeConfiguration<ProductionPackDetail>
    {
        public void Configure(EntityTypeBuilder<ProductionPackDetail> builder)
        {
            ...
            builder.Property(t => t.ProductionPackHeaderId).HasColumnName("ProductionPackHeaderId").HasColumnType("int").IsRequired();
            ...
            builder.Property(t => t.StartPackNo).HasColumnName("StartPackNo").HasColumnType("int").IsRequired();
            builder.Property(t => t.EndPackNo).HasColumnName("EndPackNo").HasColumnType("int").IsRequired();
            ...
            builder.Property(t => t.BinId).HasColumnName("BinId").HasColumnType("int").IsRequired();
            builder.Property(t => t.QualityStatusId).HasColumnName("QualityStatusId").HasColumnType("int").IsRequired();
            builder.Property(t => t.LineRemarks).HasColumnName("LineRemarks").HasColumnType("nvarchar(250)").IsRequired(false);

            builder.HasOne(t => t.LotMaster).WithMany(l => l.ProductionPackDetails).HasForeignKey(t => t.LotId).OnDelete(DeleteBehavior.Restrict);
            builder.HasOne(t => t.PackType).WithMany(p => p.ProductionPackDetails).HasForeignKey(t => t.PackTypeId).OnDelete(DeleteBehavior.Restrict);
            builder.HasOne(t => t.QualityStatusMisc).WithMany(m => m.ProductionPackDetailsAsQualityStatus).HasForeignKey(t => t.QualityStatusId).OnDelete(DeleteBehavior.Restrict);
