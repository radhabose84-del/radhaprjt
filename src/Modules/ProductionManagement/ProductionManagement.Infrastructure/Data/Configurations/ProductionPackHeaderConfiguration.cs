...
namespace ProductionManagement.Infrastructure.Data.Configurations
{
    public class ProductionPackHeaderConfiguration : IEntityTypeConfiguration<ProductionPackHeader>
    {
        public void Configure(EntityTypeBuilder<ProductionPackHeader> builder)
        {
            ...
            builder.Property(t => t.PackNo).HasColumnName("PackNo").HasColumnType("varchar(30)").IsRequired();
            builder.Property(t => t.PackDate).HasColumnName("PackDate").HasColumnType("date").IsRequired();
            ...
            builder.HasMany(t => t.ProductionPackDetails).WithOne(d => d.ProductionPackHeader).HasForeignKey(d => d.ProductionPackHeaderId).OnDelete(DeleteBehavior.Restrict);
