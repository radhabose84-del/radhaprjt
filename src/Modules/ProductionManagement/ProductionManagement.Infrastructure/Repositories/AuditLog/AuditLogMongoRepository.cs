...
namespace ProductionManagement.Infrastructure.Repositories.AuditLog
{
    public class AuditLogRepository : IAuditLogRepository
    {
        ...
        public async Task<List<AuditLogs>> GetByAuditLogNameAsync(string searchPattern)
        {
            ...
            var filter = Builders<AuditLogs>.Filter.Or(
                Builders<AuditLogs>.Filter.Regex("CreatedByName", new BsonRegularExpression(searchPattern, "i")),
                Builders<AuditLogs>.Filter.Regex("Action", new BsonRegularExpression(searchPattern, "i")),
                Builders<AuditLogs>.Filter.Regex("Details", new BsonRegularExpression(searchPattern, "i"))
            );
