...
namespace ProductionManagement.Presentation.Controllers
{
    ...
    {
        public YarnConversionHeaderController(IMediator mediator) : base(mediator) { }
        ...
        [HttpGet("by-name")]
        public async Task<IActionResult> GetYarnConversionHeaderAutoCompleteAsync([FromQuery] string? term = null)
        {
            var result = await Mediator.Send(new GetYarnConversionHeaderAutoCompleteQuery(term ?? string.Empty));
            return Ok(new
            {
                StatusCode = StatusCodes.Status200OK,
                data = result
        ...
        [HttpPost]
        public async Task<IActionResult> CreateYarnConversionHeader([FromBody] CreateYarnConversionHeaderCommand command)
        {
        ...
        [HttpPut]
        public async Task<IActionResult> UpdateYarnConversionHeader([FromBody] UpdateYarnConversionHeaderCommand command)
        {
            var result = await Mediator.Send(command);
            return Ok(new
            {
        ...
        public async Task<IActionResult> DeleteYarnConversionHeader(int id)
        {
            ...
            return Ok(new
            {
                StatusCode = StatusCodes.Status200OK,
                isSuccess = result,
                message = result ? "Yarn Conversion deleted successfully." : "Yarn Conversion not found."
            });
        }
    }
}
