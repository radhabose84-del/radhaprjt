...
namespace InventoryManagement.Application.Common.Interfaces.Item.ItemDetail.Queries
{
    ...
    {
        Task<(List<ItemListDto> Items, int TotalCount)> GetAllAsync(int? page, int? size, string? search, bool onlyActive, int? itemGroupId, int? itemCategoryId, CancellationToken ct = default);
