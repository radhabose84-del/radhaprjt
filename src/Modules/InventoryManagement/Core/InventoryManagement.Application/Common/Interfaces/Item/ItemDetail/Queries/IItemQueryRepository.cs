...
namespace InventoryManagement.Application.Common.Interfaces.Item.ItemDetail.Queries
{
    ...
    {
        Task<(List<ItemListDto> Items, int TotalCount)> GetAllAsync(int? page, int? size, string? search, bool onlyActive, int? itemGroupId, int? itemCategoryId, int? moduleId = null, int? salesGroupId = null, CancellationToken ct = default);
        ...
        Task<List<GetItemAutoCompleteDto>> GetItemAutoCompleteAsync(string searchPattern,int? itemGroupId, int? itemCategoryId,int? sourceId,int? issueRuleId, int? moduleId = null, int? salesGroupId = null, CancellationToken ct = default);
