...
namespace InventoryManagement.Application.Reports.SubStoresStock
{
    public class GetSubStockReportSummaryQueryHandler : IRequestHandler<GetSubStockReportSummaryQuery, List<SubStockSummaryDto>>
    {
        ...
        public GetSubStockReportSummaryQueryHandler(IStockReportQueryRepository istockReportQueryRepository, IMapper mapper, IMediator mediator,
            ...
            IRackLookup rackLookup, IBinLookup binLookup, IDepartmentLookup departmentLookup, IMiscMasterQueryRepository miscMasterQueryRepository)
        {
        ...
        public async Task<List<SubStockSummaryDto>> Handle(GetSubStockReportSummaryQuery request, CancellationToken cancellationToken)
        {
            ...
            var warehouseTasks = Task.FromResult<IReadOnlyList<WarehouseLookupDto>>(Array.Empty<WarehouseLookupDto>());
            var binTasks = Task.FromResult<IReadOnlyList<BinLookupDto>>(Array.Empty<BinLookupDto>());
            var rackTasks = Task.FromResult<IReadOnlyList<RackLookupDto>>(Array.Empty<RackLookupDto>());
            if (warehouseIds.Any())
            {
                warehouseTasks = _warehouseLookup.GetByIdsAsync(warehouseIds, cancellationToken);
            }
            if (binIds.Any())
            {
                binTasks = _binLookup.GetByIdsAsync(binIds, cancellationToken);
            }
            if (rackIds.Any())
            {
                rackTasks = _rackLookup.GetByIdsAsync(rackIds, cancellationToken);
            }
            var departmentTask = _departmentLookup.GetAllDepartmentAsync();

            // 6️⃣ Await all tasks together
            await Task.WhenAll(warehouseTasks, binTasks, rackTasks, departmentTask);
            ...
            var warehouseById = (await warehouseTasks)
                .Where(x => x != null)
                .ToDictionary(x => x.Id, x => x.WarehouseName);

            var binById = (await binTasks)
                .Where(b => b != null)
                .ToDictionary(b => b.Id, b => b);

            var rackById = (await rackTasks)
                .Where(r => r != null)
