...
namespace InventoryManagement.Infrastructure.Repositories.Item.ItemDetail.Queries
{
    public sealed class ItemQueryRepository : IItemQueryRepository
    {
        ...
        public async Task<List<GetItemAutoCompleteDto>> GetItemAutoCompleteAsync(string searchPattern,int? itemGroupId, int? itemCategoryId,int? sourceId,int? issueRuleId, int? moduleId = null, int? salesGroupId = null, CancellationToken ct = default)
        {
            ...
            var query = @"
                SELECT IM.Id, ItemName, ItemCode, ParentItemId,HSNId,HSNCode,GSTPercentage,IM.ItemCategoryId,IC.ItemCategoryName,IM.ItemGroupId,IG.ItemGroupName,IM.TariffNumber,
                ...
                LEFT JOIN Inventory.ItemGroup IG ON IG.Id = IM.ItemGroupId AND IG.IsDeleted = 0
