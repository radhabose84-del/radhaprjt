...
namespace PurchaseManagement.UnitTests.Validators.TnCTemplateMaster
{
    public sealed class UpdateTnCTemplateMasterValidatorTests
    {
        ...
        private void SetupHappyPath(int id = 1, string templateName = "Test Template", int moduleId = 1)
        {
            _mockQueryRepo
                .Setup(r => r.ExistsByModuleAndNameAsync(moduleId, templateName, id, It.IsAny<CancellationToken>()))
                .ReturnsAsync(false);
        }
        ...
        public async Task Validate_ValidCommand_PassesValidation()
        {
            ...
            SetupHappyPath(command.Id, command.TemplateName, command.ModuleId);
        ...
        public async Task Validate_DuplicateName_FailsValidation()
        {
            var command = TnCTemplateMasterBuilders.ValidUpdateCommand();
            _mockQueryRepo
                .Setup(r => r.ExistsByModuleAndNameAsync(command.ModuleId, command.TemplateName, command.Id, It.IsAny<CancellationToken>()))
                .ReturnsAsync(true);
