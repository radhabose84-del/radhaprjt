...
{
    public sealed class DeleteDeliveryScoreRuleCommandHandlerTests
    {
        private readonly Mock<IDeliveryScoreRuleCommandRepository> _mockCommandRepo = new(MockBehavior.Loose);
        private readonly Mock<IMediator> _mockMediator = new(MockBehavior.Loose);

        private DeleteDeliveryScoreRuleCommandHandler CreateSut() =>
        ...
        public async Task Handle_ExistingId_ReturnsTrue()
        {
            ...
            var result = await CreateSut().Handle(new DeleteDeliveryScoreRuleCommand(1), CancellationToken.None);
        ...
        {
            _mockCommandRepo.Setup(r => r.SoftDeleteAsync(1, It.IsAny<CancellationToken>())).ReturnsAsync(true);
            await CreateSut().Handle(new DeleteDeliveryScoreRuleCommand(1), CancellationToken.None);
        ...
        public async Task Handle_NonExistentId_ThrowsExceptionRules()
        {
            ...
            Func<Task> act = async () => await CreateSut().Handle(new DeleteDeliveryScoreRuleCommand(99), CancellationToken.None);
        ...
        {
            _mockCommandRepo.Setup(r => r.SoftDeleteAsync(99, It.IsAny<CancellationToken>())).ReturnsAsync(false);
            try { await CreateSut().Handle(new DeleteDeliveryScoreRuleCommand(99), CancellationToken.None); } catch { }
            _mockMediator.Verify(
                m => m.Publish(It.IsAny<AuditLogsDomainEvent>(), It.IsAny<CancellationToken>()),
                Times.Never);
        }
