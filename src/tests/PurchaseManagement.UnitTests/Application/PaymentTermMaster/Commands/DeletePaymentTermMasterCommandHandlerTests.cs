...
namespace PurchaseManagement.UnitTests.Application.PaymentTermMaster.Commands
{
    public sealed class DeletePaymentTermMasterCommandHandlerTests
    {
        ...
        public async Task Handle_ValidCommand_ReturnsTrue()
        {
            ...
            var result = await CreateSut().Handle(new DeletePaymentTermMasterCommand { Id = 1 }, CancellationToken.None);
        ...
        public async Task Handle_EntityNotFound_ThrowsExceptionRules()
        {
            ...
            var sut = CreateSut();
            Func<Task> act = async () => await sut.Handle(new DeletePaymentTermMasterCommand { Id = 999 }, CancellationToken.None);
        ...
        public async Task Handle_DeleteFails_ThrowsExceptionRules()
        {
            ...
            var sut = CreateSut();
            Func<Task> act = async () => await sut.Handle(new DeletePaymentTermMasterCommand { Id = 1 }, CancellationToken.None);
        ...
        public async Task Handle_ValidCommand_CallsDeleteOnce()
        {
            ...
            await CreateSut().Handle(new DeletePaymentTermMasterCommand { Id = 1 }, CancellationToken.None);

            _mockCommandRepo.Verify(r => r.DeleteAsync(1), Times.Once);
        }

        [Fact]
        public async Task Handle_EntityNotFound_DoesNotCallDelete()
        {
            _mockQueryRepo
                .Setup(r => r.GetByIdAsync(It.IsAny<int>()))
                .Returns(Task.FromResult<PaymentTermMasterDto>(null!));
            ...
            var sut = CreateSut();
            try { await sut.Handle(new DeletePaymentTermMasterCommand { Id = 999 }, CancellationToken.None); }
            catch { /* expected */ }
