...
public class ForecloseBlanketPOCommandHandlerTests
{
    private readonly Mock<IBlanketPOCommandRepository> _mockCommandRepo = new(MockBehavior.Strict);
    private readonly Mock<IMediator> _mockMediator = new(MockBehavior.Strict);

    private ForecloseBlanketPOCommandHandler CreateSut() => new(_mockCommandRepo.Object, _mockMediator.Object);
    ...
    public async Task Handle_ValidId_ReturnsTrue_AndPublishes()
    {
        ...
        var result = await CreateSut().Handle(new ForecloseBlanketPOCommand(1), CancellationToken.None);

        result.Should().BeTrue();
        _mockMediator.Verify(
            m => m.Publish(It.Is<AuditLogsDomainEvent>(e => e.ActionCode == "BLANKET_RELEASE_PO_FORECLOSE"),
    ...
    public async Task Handle_NotFound_ThrowsException()
    {
        ...
        var act = async () => await CreateSut().Handle(new ForecloseBlanketPOCommand(99), CancellationToken.None);
