...
{
    public sealed class DeleteFreightRfqCommandHandlerTests
    {
        private readonly Mock<IFreightRfqCommandRepository> _mockCommandRepo = new(MockBehavior.Loose);
        private readonly Mock<IMediator> _mockMediator = new(MockBehavior.Loose);

        private DeleteFreightRfqCommandHandler CreateSut() =>
        ...
        public async Task Handle_ExistingId_ReturnsTrue()
        {
            ...
            var result = await CreateSut().Handle(new DeleteFreightRfqCommand(1), CancellationToken.None);
        ...
        public async Task Handle_NonExistentId_ThrowsExceptionRules()
        {
            ...
            Func<Task> act = async () => await CreateSut().Handle(new DeleteFreightRfqCommand(99), CancellationToken.None);
        ...
        public async Task Handle_NonExistentId_DoesNotPublishAuditEvent()
        {
            ...
            try { await CreateSut().Handle(new DeleteFreightRfqCommand(99), CancellationToken.None); }
            catch { /* expected */ }
