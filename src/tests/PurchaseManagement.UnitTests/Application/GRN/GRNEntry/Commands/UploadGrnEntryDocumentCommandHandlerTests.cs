...
{
    public sealed class UploadGrnEntryDocumentCommandHandlerTests
    {
        private readonly Mock<IGRNEntryQueryRepository> _mockRepo = new(MockBehavior.Loose);

        private UploadGrnEntryDocumentCommandHandler CreateSut() =>
            new(_mockRepo.Object);

        [Fact]
        public async Task Handle_NullFile_ThrowsValidationException()
        {
            var command = new UploadGrnEntryDocumentCommand { File = null };
        ...
        public async Task Handle_EmptyFile_ThrowsValidationException()
        {
            ...
            var command = new UploadGrnEntryDocumentCommand { File = mockFile.Object };
