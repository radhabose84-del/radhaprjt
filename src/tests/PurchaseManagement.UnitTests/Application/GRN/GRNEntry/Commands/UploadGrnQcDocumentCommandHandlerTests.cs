...
{
    public sealed class UploadGrnQcDocumentCommandHandlerTests
    {
        private readonly Mock<IGRNEntryQueryRepository> _mockRepo = new(MockBehavior.Loose);

        private UploadGrnQcDocumentCommandHandler CreateSut() =>
            new(_mockRepo.Object);

        [Fact]
        public async Task Handle_NullFile_ThrowsValidationException()
        {
            var command = new UploadGrnQcDocumentCommand { File = null };
        ...
        public async Task Handle_EmptyFile_ThrowsValidationException()
        {
            ...
            var command = new UploadGrnQcDocumentCommand { File = mockFile.Object };
