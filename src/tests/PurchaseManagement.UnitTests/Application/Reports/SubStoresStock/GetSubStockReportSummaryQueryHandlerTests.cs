...
namespace PurchaseManagement.UnitTests.Application.Reports.SubStoresStock
{
    public sealed class GetSubStockReportSummaryQueryHandlerTests
    {
        ...
        {
            var query = new GetSubStockReportSummaryQuery();
            _mockRepo.Setup(r => r.GetSubStoresStockSummaryAsync(
                It.IsAny<int?>(), It.IsAny<int?>(), It.IsAny<int?>(), It.IsAny<int?>(), It.IsAny<int?>()))
                .ReturnsAsync(new List<SubStockSummaryDto>());
            _mockMapper.Setup(m => m.Map<List<SubStockSummaryDto>>(It.IsAny<object>()))
                .Returns(new List<SubStockSummaryDto>());
            SetupEmptyLookups();
