...
public class CancelContractPOCommandHandlerTests
{
    private readonly Mock<IContractPOCommandRepository> _mockCommandRepo = new(MockBehavior.Strict);
    private readonly Mock<IMediator> _mockMediator = new(MockBehavior.Strict);

    private CancelContractPOCommandHandler CreateSut() => new(_mockCommandRepo.Object, _mockMediator.Object);
    ...
    public async Task Handle_ValidId_ReturnsTrue_AndPublishes()
    {
        ...
        var result = await CreateSut().Handle(new CancelContractPOCommand(1), CancellationToken.None);

        result.Should().BeTrue();
        _mockMediator.Verify(
            m => m.Publish(It.Is<AuditLogsDomainEvent>(e => e.ActionCode == "CONTRACT_PO_CANCEL"),
    ...
    public async Task Handle_NotFound_ThrowsException()
    {
        ...
        var act = async () => await CreateSut().Handle(new CancelContractPOCommand(99), CancellationToken.None);
