...
public sealed class UpdateReturnReasonCommandHandlerTests
{
    ...
    private UpdateReturnReasonCommandHandler CreateSut() =>
    ...
    public async Task Handle_ValidCommand_ReturnsDto()
    {
        ...
        _mockMapper.Setup(m => m.Map<DomainReturnReason>(It.IsAny<UpdateReturnReasonCommand>())).Returns(entity);
        _mockCommandRepo.Setup(r => r.UpdateAsync(It.IsAny<DomainReturnReason>(), It.IsAny<CancellationToken>())).ReturnsAsync(entity);
        _mockQueryRepo.Setup(r => r.GetByIdAsync(It.IsAny<int>(), It.IsAny<CancellationToken>())).ReturnsAsync(ReturnReasonBuilders.ValidDto());

        var result = await CreateSut().Handle(ReturnReasonBuilders.ValidUpdateCommand(), CancellationToken.None);
