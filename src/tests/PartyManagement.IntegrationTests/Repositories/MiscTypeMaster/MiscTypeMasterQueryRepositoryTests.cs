...
namespace PartyManagement.IntegrationTests.Repositories.MiscTypeMaster
{
    ...
    public sealed class MiscTypeMasterQueryRepositoryTests
    {
        ...
        private async Task<int> SeedEntityAsync(string code = "PARTY_TYPE", string description = "Party Type")
        {
        ...
        {
            await ClearTableAsync();
            await SeedEntityAsync("PARTY_TYPE", "Party Type");

            var (items, _) = await CreateQueryRepo().GetAllMiscTypeMasterAsync(1, 10, null);

            items[0].MiscTypeCode.Should().Be("PARTY_TYPE");
            items[0].Description.Should().Be("Party Type");
        }
        ...
        {
            await ClearTableAsync();
            var id = await SeedEntityAsync("PARTY_TYPE", "Party Type");

            var result = await CreateQueryRepo().GetByIdAsync(id);

            result.Should().NotBeNull();
            result.Id.Should().Be(id);
            result.MiscTypeCode.Should().Be("PARTY_TYPE");
        }
        ...
        {
            await ClearTableAsync();
            await SeedEntityAsync("PARTY_TYPE");

            var exists = await CreateQueryRepo().AlreadyExistsAsync("PARTY_TYPE");
        ...
        {
            await ClearTableAsync();
            var id = await SeedEntityAsync("PARTY_TYPE");
        ...
        public async Task AlreadyExistsAsync_Should_Exclude_Self_When_Id_Provided()
        {
            ...
            var exists = await CreateQueryRepo().AlreadyExistsAsync("PARTY_TYPE", id);
        ...
        public async Task GetMiscTypeMaster_Should_Return_Matching_Records()
        {
            await ClearTableAsync();
            await SeedEntityAsync("PARTY_TYPE", "Party Type");
            await SeedEntityAsync("PARTY_STATUS", "Party Status");
            await SeedEntityAsync("OTHER", "Other");

            var results = await CreateQueryRepo().GetMiscTypeMaster("PARTY");
