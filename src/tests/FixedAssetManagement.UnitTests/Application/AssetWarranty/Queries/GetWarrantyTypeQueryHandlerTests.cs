...
{
    public sealed class GetWarrantyTypeQueryHandlerTests
    {
        private readonly Mock<IAssetWarrantyQueryRepository> _mockRepo = new(MockBehavior.Strict);
        private readonly Mock<IMapper> _mockMapper = new(MockBehavior.Loose);

        private GetWarrantyTypeQueryHandler CreateSut() =>
        ...
        public async Task Handle_ReturnsSuccess()
        {
            ...
            _mockRepo
                .Setup(r => r.GetWarrantyTypeAsync())
                .ReturnsAsync(entities);
            _mockMapper
                .Setup(m => m.Map<List<GetMiscMasterDto>>(It.IsAny<object>()))
                .Returns(dtos);

            var result = await CreateSut().Handle(new GetWarrantyTypeQuery(), CancellationToken.None);
