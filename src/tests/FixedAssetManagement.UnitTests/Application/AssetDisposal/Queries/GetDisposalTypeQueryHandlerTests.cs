...
{
    public sealed class GetDisposalTypeQueryHandlerTests
    {
        private readonly Mock<IAssetDisposalQueryRepository> _mockRepo = new(MockBehavior.Strict);
        private readonly Mock<IMapper> _mockMapper = new(MockBehavior.Loose);

        private GetDisposalTypeQueryHandler CreateSut() =>
        ...
        public async Task Handle_ReturnsSuccess()
        {
            ...
            _mockRepo
                .Setup(r => r.GetDisposalType())
                .ReturnsAsync(entities);
            _mockMapper
                .Setup(m => m.Map<List<GetMiscMasterDto>>(It.IsAny<object>()))
                .Returns(dtos);

            var result = await CreateSut().Handle(new GetDisposalTypeQuery(), CancellationToken.None);
