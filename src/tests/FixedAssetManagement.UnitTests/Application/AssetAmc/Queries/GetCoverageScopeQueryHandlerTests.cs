...
{
    public sealed class GetCoverageScopeQueryHandlerTests
    {
        private readonly Mock<IAssetAmcQueryRepository> _mockRepo = new(MockBehavior.Strict);
        private readonly Mock<IMapper> _mockMapper = new(MockBehavior.Loose);

        private GetCoverageScopeQueryHandler CreateSut() =>
        ...
        public async Task Handle_ReturnsSuccess()
        {
            ...
            _mockRepo
                .Setup(r => r.GetCoverageScopeAsync())
                .ReturnsAsync(entities);
            _mockMapper
                .Setup(m => m.Map<List<GetMiscMasterDto>>(It.IsAny<object>()))
                .Returns(dtos);

            var result = await CreateSut().Handle(new GetCoverageScopeQuery(), CancellationToken.None);
