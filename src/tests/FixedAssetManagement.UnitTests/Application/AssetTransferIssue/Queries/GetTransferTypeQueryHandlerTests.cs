...
{
    public sealed class GetTransferTypeQueryHandlerTests
    {
        private readonly Mock<IAssetTransferQueryRepository> _mockRepo = new(MockBehavior.Strict);
        private readonly Mock<IMapper> _mockMapper = new(MockBehavior.Loose);

        private GetTransferTypeQueryHandler CreateSut() =>
        ...
        public async Task Handle_ReturnsSuccess()
        {
            ...
            _mockRepo
                .Setup(r => r.GetTransferTypeAsync())
                .ReturnsAsync(entities);
            _mockMapper
                .Setup(m => m.Map<List<GetMiscMasterDto>>(It.IsAny<object>()))
                .Returns(dtos);

            var result = await CreateSut().Handle(new GetTransferTypeQuery(), CancellationToken.None);
