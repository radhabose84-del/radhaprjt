...
{
    public sealed class UploadAssetMasterGeneralCommandValidatorTests
    {
        private UploadAssetMasterGeneralCommandValidator CreateValidator() => new();
        ...
        public async Task Validate_OversizedFile_FailsValidation()
        {
            var command = new UploadFileAssetMasterGeneralCommand
            {
                File = CreateMockFile("test.jpg", 5 * 1024 * 1024)
            };
