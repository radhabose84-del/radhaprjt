...
namespace InventoryManagement.IntegrationTests.Repositories.Outbox
{
    ...
    public sealed class OutboxRepositoryTests
    {
        ...
        {
            await ClearAsync();
            await CreateRepo().AddAsync(BuildMessage(nextRetryAt: DateTimeOffset.UtcNow.AddMinutes(10)));
            await CreateRepo().AddAsync(BuildMessage(nextRetryAt: null));
