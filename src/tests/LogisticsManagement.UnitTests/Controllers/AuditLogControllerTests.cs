...
namespace LogisticsManagement.UnitTests.Controllers
{
    public sealed class AuditLogControllerTests
    {
        ...
        [Fact]
        public async Task GetAuditLogAutoComplete_ReturnsOkResult()
        {
            _mockMediator
        ...
        [Fact]
        public async Task GetAuditLogAutoComplete_CallsMediatorSend_Once()
        {
            _mockMediator
                .Setup(m => m.Send(It.IsAny<GetAuditLogBySearchPatternQuery>(), It.IsAny<CancellationToken>()))
                .ReturnsAsync(new ApiResponseDTO<List<AuditLogDto>>
            ...
            await CreateSut().GetAuditLogAutoCompleteAsync("test");
            _mockMediator.Verify(
