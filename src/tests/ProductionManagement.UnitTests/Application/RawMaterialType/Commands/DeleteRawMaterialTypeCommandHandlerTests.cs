...
{
    public sealed class DeleteRawMaterialTypeCommandHandlerTests
    {
        private readonly Mock<IRawMaterialTypeCommandRepository> _mockCommandRepo = new(MockBehavior.Strict);
        private readonly Mock<IMediator> _mockMediator = new(MockBehavior.Loose);

        private DeleteRawMaterialTypeCommandHandler CreateSut() =>
        ...
        public async Task Handle_ValidId_ReturnsTrue()
        {
            ...
            var result = await CreateSut().Handle(new DeleteRawMaterialTypeCommand(7), CancellationToken.None);
        ...
        public async Task Handle_ValidId_CallsSoftDeleteOnce()
        {
            ...
            await CreateSut().Handle(new DeleteRawMaterialTypeCommand(7), CancellationToken.None);
        ...
        public async Task Handle_ValidId_PublishesAuditEvent()
        {
            ...
            _mockMediator.Verify(
                m => m.Publish(It.Is<AuditLogsDomainEvent>(e => e.ActionCode == "RAWMATERIALTYPE_DELETE"), It.IsAny<CancellationToken>()),
                Times.Once);
        }
    }
}
