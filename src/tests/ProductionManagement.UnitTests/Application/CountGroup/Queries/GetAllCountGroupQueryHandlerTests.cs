...
{
    public sealed class GetAllCountGroupQueryHandlerTests
    {
        private readonly Mock<ICountGroupQueryRepository> _mockQueryRepo = new(MockBehavior.Strict);
        private readonly Mock<IMapper> _mockMapper = new(MockBehavior.Loose);
        private readonly Mock<IMediator> _mockMediator = new(MockBehavior.Loose);

        private GetAllCountGroupQueryHandler CreateSut() =>
            new(_mockQueryRepo.Object, _mockMapper.Object, _mockMediator.Object);

        [Fact]
        public async Task Handle_ReturnsSuccess()
        {
            var dtoList = new List<CountGroupDto> { new() };
            _mockQueryRepo.Setup(r => r.GetAllAsync(1, 10, null)).ReturnsAsync((dtoList, 1));
            _mockMapper.Setup(m => m.Map<List<CountGroupDto>>(It.IsAny<object>())).Returns(dtoList);
            _mockMediator.Setup(m => m.Publish(It.IsAny<AuditLogsDomainEvent>(), It.IsAny<CancellationToken>())).Returns(Task.CompletedTask);

            var result = await CreateSut().Handle(new GetAllCountGroupQuery { PageNumber = 1, PageSize = 10 }, CancellationToken.None);
        ...
        {
            var dtoList = new List<CountGroupDto>();
        ...
        public async Task Handle_ReturnsPaginationMetadata()
        {
            ...
            var result = await CreateSut().Handle(new GetAllCountGroupQuery { PageNumber = 2, PageSize = 5, SearchTerm = "test" }, CancellationToken.None);
