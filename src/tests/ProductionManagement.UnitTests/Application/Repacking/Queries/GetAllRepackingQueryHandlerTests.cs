...
namespace ProductionManagement.UnitTests.Application.Repacking.Queries
{
    public sealed class GetAllRepackingHeaderQueryHandlerTests
    {
        ...
        public async Task Handle_ReturnsSuccess()
        {
            ...
            _mockQueryRepo.Setup(r => r.GetAllAsync(1, 10, (string?)null)).ReturnsAsync((dtoList, 1));
        ...
        {
            _mockQueryRepo.Setup(r => r.GetAllAsync(1, 10, (string?)null)).ReturnsAsync((new List<RepackingHeaderDto>(), 0));
            _mockMediator.Setup(m => m.Publish(It.IsAny<AuditLogsDomainEvent>(), It.IsAny<CancellationToken>())).Returns(Task.CompletedTask);
