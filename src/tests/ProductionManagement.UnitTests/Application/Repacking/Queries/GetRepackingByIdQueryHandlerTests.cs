...
namespace ProductionManagement.UnitTests.Application.Repacking.Queries
{
    public sealed class GetRepackingByIdQueryHandlerTests
    {
        ...
        public async Task Handle_ExistingId_ReturnsDto()
        {
            ...
            _mockMapper.Setup(m => m.Map<RepackingHeaderDto>(It.IsAny<object>())).Returns(dto);
            _mockMediator.Setup(m => m.Publish(It.IsAny<AuditLogsDomainEvent>(), It.IsAny<CancellationToken>())).Returns(Task.CompletedTask);

            var result = await CreateSut().Handle(new GetRepackingByIdQuery { Id = 1 }, CancellationToken.None);
        ...
        public async Task Handle_NotFound_ReturnsNull()
        {
            ...
            var result = await CreateSut().Handle(new GetRepackingByIdQuery { Id = 999 }, CancellationToken.None);
