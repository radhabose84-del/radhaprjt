...
namespace ProductionManagement.UnitTests.Application.PackType.Commands
{
    public sealed class DeletePackTypeCommandHandlerTests
    {
        ...
        public async Task Handle_ValidId_ReturnsTrue()
        {
            ...
            var result = await CreateSut().Handle(new DeletePackTypeCommand(1), CancellationToken.None);
        ...
        public async Task Handle_ValidId_CallsSoftDeleteOnce()
        {
            ...
            await CreateSut().Handle(new DeletePackTypeCommand(1), CancellationToken.None);
