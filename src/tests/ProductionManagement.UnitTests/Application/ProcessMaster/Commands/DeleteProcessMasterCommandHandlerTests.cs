...
{
    public sealed class DeleteProcessMasterCommandHandlerTests
    {
        private readonly Mock<IProcessMasterCommandRepository> _mockCommandRepo = new(MockBehavior.Strict);
        private readonly Mock<IMediator> _mockMediator = new(MockBehavior.Loose);

        private DeleteProcessMasterCommandHandler CreateSut() =>
        ...
        public async Task Handle_ValidId_ReturnsTrue()
        {
            ...
            var result = await CreateSut().Handle(new DeleteProcessMasterCommand(1), CancellationToken.None);
        ...
        public async Task Handle_ValidId_CallsSoftDeleteOnce()
        {
            ...
            await CreateSut().Handle(new DeleteProcessMasterCommand(1), CancellationToken.None);
