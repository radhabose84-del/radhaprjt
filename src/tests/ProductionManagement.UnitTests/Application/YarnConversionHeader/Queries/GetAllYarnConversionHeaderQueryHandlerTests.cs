...
{
    public sealed class GetAllYarnConversionHeaderQueryHandlerTests
    {
        ...
        private GetAllYarnConversionHeaderQueryHandler CreateSut() =>
        ...
        {
            var dtoList = new List<YarnConversionHeaderDto> { BuildDto() };
            _mockQueryRepo.Setup(r => r.GetAllAsync(1, 10, null)).ReturnsAsync((dtoList, 1));
            _mockMediator.Setup(m => m.Publish(It.IsAny<AuditLogsDomainEvent>(), It.IsAny<CancellationToken>())).Returns(Task.CompletedTask);

            var result = await CreateSut().Handle(
                new GetAllYarnConversionHeaderQuery { PageNumber = 1, PageSize = 10 },
        ...
        public async Task Handle_ReturnsPaginationMetadata()
        {
            ...
            var result = await CreateSut().Handle(
                new GetAllYarnConversionHeaderQuery { PageNumber = 2, PageSize = 5, SearchTerm = "search" },
                CancellationToken.None);
        ...
        {
            _mockQueryRepo.Setup(r => r.GetAllAsync(1, 10, null)).ReturnsAsync((new List<YarnConversionHeaderDto>(), 0));
            _mockMediator.Setup(m => m.Publish(It.IsAny<AuditLogsDomainEvent>(), It.IsAny<CancellationToken>())).Returns(Task.CompletedTask);
