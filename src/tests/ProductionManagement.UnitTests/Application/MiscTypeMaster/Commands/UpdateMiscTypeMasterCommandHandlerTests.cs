...
namespace ProductionManagement.UnitTests.Application.MiscTypeMaster.Commands
{
    public sealed class UpdateMiscTypeMasterCommandHandlerTests
    {
        ...
        {
            SetupHappyPath();
            var result = await CreateSut().Handle(new UpdateMiscTypeMasterCommand { Id = 1 }, CancellationToken.None);
        ...
        {
            SetupHappyPath();
            await CreateSut().Handle(new UpdateMiscTypeMasterCommand { Id = 1 }, CancellationToken.None);
            _mockCommandRepo.Verify(r => r.UpdateAsync(It.IsAny<ProductionManagement.Domain.Entities.MiscTypeMaster>()), Times.Once);
        }
