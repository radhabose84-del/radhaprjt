...
namespace ProductionManagement.UnitTests.Application.MiscMaster.Queries
{
    public sealed class GetMiscMasterByIdQueryHandlerTests
    {
        ...
        {
            var dto = new MiscMasterDto { Id = 1 };
            _mockQueryRepo.Setup(r => r.GetByIdAsync(1)).ReturnsAsync(dto);
            _mockMapper.Setup(m => m.Map<MiscMasterDto>(It.IsAny<object>())).Returns(dto);
        ...
        {
            _mockQueryRepo.Setup(r => r.GetByIdAsync(999)).ReturnsAsync((MiscMasterDto?)null);

            var result = await CreateSut().Handle(new GetMiscMasterByIdQuery { Id = 999 }, CancellationToken.None);
