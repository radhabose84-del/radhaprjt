...
namespace PurchaseManagement.QATests.Tests.Security;

// ─────────────────────────────────────────────────────────────────────────────
// Phase 3 — Security: authentication-bypass checks (PurchaseManagement surface).
//
...
public sealed class AuthBypassQATests
{
    ...
    private const string ProtectedGet  = "/api/purchase/miscmaster?PageNumber=1&PageSize=5";
    private const string ProtectedPost = "/api/purchase/miscmaster";
    ...
    public async Task TC007_Post_NoToken_Returns401()
    {
        ...
        {
            description = "Sec Bypass Attempt"
        });
        resp.StatusCode.Should().Be(HttpStatusCode.Unauthorized);
    }
}
