namespace PurchaseManagement.QATests.Tests.ReturnReason;

// ─────────────────────────────────────────────────────────────────────────────
// ReturnReason — live-server QA suite (CRUD lifecycle + negatives).
//
// Contract verified against source (2026-06-17 — ReturnReasonController):
//   Route = [Route("api/[controller]")] → /api/ReturnReason
//   POST   /api/ReturnReason   { code, description, returnTypeId, isReplacementOverride?,
//                              isDebitNoteOverride?, isQcMandatoryOverride? }   (inline validator → real 400)
//   PUT    /api/ReturnReason    { id, description, ...(no code), isActive(int) }   (code immutable; inline 400)
//   DELETE /api/ReturnReason/{id}   (id bound from ROUTE; inline delete validator → real 400)
//   GET    /api/ReturnReason?PageNumber=&PageSize=&SearchTerm=
//   GET    /api/ReturnReason/{id}   (always 200; data wraps ApiResponse)
//   GET    /api/ReturnReason/by-name?term=
//   GET    /api/ReturnReason/by-return-type/{returnTypeId:int}
//
// Key facts that shaped assertions:
//   • returnTypeId → /api/ReturnType FK (REQUIRED). Resolved at runtime via FirstIdAsync; if none
//     exists a fresh ReturnType is POSTed. If still 0 the create-happy self-skips.
//   • Create returns 201 envelope inside Ok(200) → CreatedId(). Create/Update/Delete validators inline → real 400.
//   • code is REQUIRED + immutable + unique.
// ─────────────────────────────────────────────────────────────────────────────

[Collection("ReturnReasonCollection")]
[TestCaseOrderer("Shared.QAInfrastructure.Infrastructure.PriorityOrderer", "Shared.QAInfrastructure")]
public sealed class ReturnReasonQATests
{
    private readonly QAServerFixture _f;
    private const string BaseRoute = "/api/ReturnReason";
    ...
    public ReturnReasonQATests(QAServerFixture fixture) => _f = fixture;

    private string NewCode() => _f.EntityCode[..10];

    // Resolve an existing ReturnType id; create one if the clone has none.
    private async Task<int> ResolveReturnTypeIdAsync()
    {
        var id = await QAHelper.FirstIdAsync(_f.Client, ReturnTypeRoute);
        if (id > 0) return id;

        var create = await _f.Client.PostAsJsonAsync(ReturnTypeRoute, new
        {
            code = "RT" + _f.EntityCode[..8],
            description = "QA Parent Return Type",
            isReplacementApplicable = false,
            isQcMandatory = false
        });
        if (!create.IsSuccessStatusCode) return 0;

        var doc = await QAHelper.ParseAsync(create);
        try { return doc.RootElement.CreatedId(); } catch { return 0; }
    }
    ...
    public async Task TC001_Create_HappyPath_Returns200_And_CapturesId()
    {
        ...
        _returnTypeId = await ResolveReturnTypeIdAsync();
        ...
        {
            code = _createdCode,
            description = "QA Test Return Reason",
    ...
    public async Task TC002_Create_NoAuthToken_Returns401()
    {
        ...
        {
            code = "NOAUTH01",
            description = "No Auth Return Reason",
            returnTypeId = 1
        });
    ...
    public async Task TC003_Create_CodeEmpty_Returns400()
    {
        var resp = await _f.Client.PostAsJsonAsync(BaseRoute, new
        {
            code = "",
            description = "QA Test Return Reason",
            returnTypeId = await ResolveReturnTypeIdAsync()
        });
    ...
    [Fact, TestPriority(5)]
    public async Task TC005_Create_ReturnTypeIdMissing_Returns400()
    {
        var resp = await _f.Client.PostAsJsonAsync(BaseRoute, new
        {
            code = NewCode(),
    ...
    {
        if (_f.CreatedId == 0 || _returnTypeId == 0) return; // create-happy skipped
    ...
    public async Task TC023_GetByReturnType_Returns200()
    {
        if (_returnTypeId == 0) return; // FK unresolved

        var resp = await _f.Client.GetAsync($"{BaseRoute}/by-return-type/{_returnTypeId}");
        await QAHelper.AssertOkAsync(resp);
    }
    ...
    public async Task TC050_Update_HappyPath_Returns200()
    {
        ...
        // Live contract: Update requires returnTypeId echoed back even though it is immutable.
        var resp = await _f.Client.PutAsJsonAsync(BaseRoute, new
        {
            id = _f.CreatedId,
            description = "QA Updated Return Reason",
            returnTypeId = _returnTypeId,
            isActive = 1
        });
