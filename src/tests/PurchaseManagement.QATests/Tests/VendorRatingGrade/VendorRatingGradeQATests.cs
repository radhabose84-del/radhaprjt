namespace PurchaseManagement.QATests.Tests.VendorRatingGrade;

// ─────────────────────────────────────────────────────────────────────────────
// VendorRatingGrade — live-server QA suite (CRUD lifecycle + negatives).  CLEAN master (actionTypeId optional).
//
// Contract verified against source (2026-06-17 — VendorRatingGradeController):
//   Route = [Route("api/[controller]")] → /api/VendorRatingGrade
//   POST   /api/VendorRatingGrade   { gradeCode, gradeName, minScore, maxScore, actionDescription?,
//                                   actionTypeId?, sortOrder }
//   PUT    /api/VendorRatingGrade    { id, gradeName, ...(no gradeCode), isActive(int) }   (gradeCode immutable)
//   DELETE /api/VendorRatingGrade/{id}   (id bound from ROUTE)
//   GET    /api/VendorRatingGrade?PageNumber=&PageSize=&SearchTerm=
//   GET    /api/VendorRatingGrade/{id}   (always 200; data wraps ApiResponse)
//   GET    /api/VendorRatingGrade/by-name?term=
//
// Key facts that shaped assertions:
//   • actionTypeId is OPTIONAL → no required FK; fully self-contained.
//   • All write endpoints return Ok(200) with isSuccess inside the body (pipeline ValidationBehavior) →
//     negatives (duplicate code, minScore>maxScore) surface as 200+isSuccess=false OR 400 → tolerated (200,400).
//   • Create returns 200 envelope; id captured via CreatedId().
//   • gradeCode is immutable + unique; maxScore >= minScore.
// ─────────────────────────────────────────────────────────────────────────────

[Collection("VendorRatingGradeCollection")]
[TestCaseOrderer("Shared.QAInfrastructure.Infrastructure.PriorityOrderer", "Shared.QAInfrastructure")]
public sealed class VendorRatingGradeQATests
{
    private readonly QAServerFixture _f;
    private const string BaseRoute = "/api/VendorRatingGrade";

    private static string _createdCode = string.Empty;

    public VendorRatingGradeQATests(QAServerFixture fixture) => _f = fixture;
    ...
    public async Task TC001_Create_HappyPath_Returns200_And_CapturesId()
    {
        ...
        {
            gradeCode = _createdCode,
            gradeName = "QA Test Grade",
            minScore = 0m,
            maxScore = 50m,
    ...
    [Fact, TestPriority(4)]
    public async Task TC004_Create_NameEmpty_Returns200_Or_400()
    {
        var resp = await _f.Client.PostAsJsonAsync(BaseRoute, new
        {
            gradeCode = NewCode(),
    ...
    [Fact, TestPriority(5)]
    public async Task TC005_Create_MinScoreGreaterThanMaxScore_Returns200_Or_400()
    {
        var resp = await _f.Client.PostAsJsonAsync(BaseRoute, new
        {
            gradeCode = NewCode(),
    ...
    [Fact, TestPriority(6)]
    public async Task TC006_Create_DuplicateCode_Returns200_Or_400()
    {
        var resp = await _f.Client.PostAsJsonAsync(BaseRoute, new
        {
            gradeCode = _createdCode,
    ...
    [Fact, TestPriority(7)]
    public async Task TC007_Create_EmptyBody_Returns200_Or_400()
    {
        var resp = await _f.Client.PostAsJsonAsync(BaseRoute, new { });
        ((int)resp.StatusCode).Should().BeOneOf(200, 400);
    }
    ...
    // ─────────────────────────────────────────────────────────────────────────
    // SECTION 5 — UPDATE  (gradeCode immutable; isActive int)
    // ─────────────────────────────────────────────────────────────────────────
