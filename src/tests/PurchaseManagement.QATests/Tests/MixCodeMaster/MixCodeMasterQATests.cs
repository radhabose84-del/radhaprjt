namespace PurchaseManagement.QATests.Tests.MixCodeMaster;

// ─────────────────────────────────────────────────────────────────────────────
// MixCodeMaster — live-server QA suite (CRUD lifecycle + negatives).
//
// Contract verified against source (2026-06-17 — MixCodeMasterController):
//   POST   /api/MixCodeMaster            { mixCode, mixCodeDesc }  (both REQUIRED; mixCode alphanumeric/max20/unique)
//   PUT    /api/MixCodeMaster             { id, mixCodeDesc, isActive } (mixCode immutable)
//   DELETE /api/MixCodeMaster/{id}        (id bound from ROUTE)
//   GET    /api/MixCodeMaster?PageNumber=&PageSize=&SearchTerm=
//   GET    /api/MixCodeMaster/{id}        (always 200; data wraps ApiResponse)
//   GET    /api/MixCodeMaster/by-name?term=
//
// Key facts that shaped assertions:
//   • No FK — fully self-contained clean master.
//   • ALL write endpoints always return Ok(200) with isSuccess/result inside the body —
//     validation failures surface as 200+isSuccess=false (pipeline ValidationBehavior) OR 400.
//     Negative create/update/delete are therefore tolerated as (200,400).
//   • Create returns 200 envelope; id captured via CreatedId() (bare-int data).
// ─────────────────────────────────────────────────────────────────────────────

[Collection("MixCodeMasterCollection")]
[TestCaseOrderer("Shared.QAInfrastructure.Infrastructure.PriorityOrderer", "Shared.QAInfrastructure")]
public sealed class MixCodeMasterQATests
{
    private readonly QAServerFixture _f;
    private const string BaseRoute = "/api/MixCodeMaster";

    private const string TestDesc = "QA Test Mix Code";

    private static string _createdCode = string.Empty;

    public MixCodeMasterQATests(QAServerFixture fixture) => _f = fixture;
    ...
    public async Task TC001_Create_HappyPath_Returns200_And_CapturesId()
    {
        ...
        {
            mixCode = _createdCode,
            mixCodeDesc = TestDesc
        });
    ...
    [Fact, TestPriority(4)]
    public async Task TC004_Create_DescEmpty_Returns200_Or_400()
    {
        var resp = await _f.Client.PostAsJsonAsync(BaseRoute, new
        {
            mixCode = NewCode(),
    ...
    [Fact, TestPriority(5)]
    public async Task TC005_Create_CodeTooLong_Returns200_Or_400()
    {
        var resp = await _f.Client.PostAsJsonAsync(BaseRoute, new
        {
            mixCode = new string('A', 50), // exceeds max 20
            mixCodeDesc = TestDesc
        });
    ...
    // ─────────────────────────────────────────────────────────────────────────
    // SECTION 5 — UPDATE  (mixCode immutable)
    // ─────────────────────────────────────────────────────────────────────────
    ...
    public async Task TC050_Update_HappyPath_Returns200()
    {
        ...
        {
            id = _f.CreatedId,
            mixCodeDesc = "QA Updated Mix Code",
            isActive = 1
        });
