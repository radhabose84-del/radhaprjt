...
namespace InventoryManagement.QATests.Tests.Security;

// ─────────────────────────────────────────────────────────────────────────────
// Phase 3 — Security: authentication-bypass checks (InventoryManagement surface).
//
...
public sealed class AuthBypassQATests
{
    ...
    private const string ProtectedGet  = "/api/itemgroup?PageNumber=1&PageSize=5";
    private const string ProtectedPost = "/api/itemgroup";
    ...
    public async Task TC007_Post_NoToken_Returns401()
    {
        ...
        {
            itemGroupCode = "SEC01",
            itemGroupName = "Sec Bypass Attempt"
        });
        resp.StatusCode.Should().Be(HttpStatusCode.Unauthorized);
    }
}
