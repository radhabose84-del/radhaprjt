namespace FixedAssetManagement.QATests.Tests.MiscTypeMaster;

// FAM MiscTypeMaster — /api/fam/MiscTypeMaster   (NOTE the /fam prefix)
// POST { miscTypeCode, description? } → DTO
// PUT  { id, miscTypeCode, description?, isActive }
// DELETE /{id}; GET ?Page..; GET /{id}; GET /by-name?name=

[Collection("FamMiscTypeMasterCollection")]
[TestCaseOrderer("Shared.QAInfrastructure.Infrastructure.PriorityOrderer", "Shared.QAInfrastructure")]
public sealed class MiscTypeMasterQATests
{
    private readonly QAServerFixture _f;
    private const string BaseRoute = "/api/fam/MiscTypeMaster";
    ...
    {
        var resp = await _f.AnonymousClient.PutAsJsonAsync(BaseRoute, new { id = _f.CreatedId, miscTypeCode = "X", isActive = (byte)1 });
        resp.StatusCode.Should().Be(HttpStatusCode.Unauthorized);
    }
