...
namespace MaintenanceManagement.IntegrationTests.Repositories.Outbox
{
    /// <summary>
    /// Integration tests for OutboxRepository.
    /// OutboxMessage is a standalone entity (no FK dependencies on Maintenance domain tables).
    ...
    public sealed class OutboxRepositoryTests
    {
        ...
        private async Task ClearOutboxAsync()
        {
            ...
            await ctx.Database.ExecuteSqlRawAsync("DELETE FROM maintenance.OutboxMessages");
        }
        ...
        public async Task AddAsync_Should_Persist_Fields_Correctly()
        {
            ...
            saved.ModuleName.Should().Be("MaintenanceManagement");
        ...
        public async Task GetByCorrelationIdAsync_Should_Return_Null_When_NotFound()
        {
            ...
            var result = await CreateRepository(ctx).GetByCorrelationIdAsync(Guid.NewGuid());
        ...
        public async Task DeleteOldMessagesAsync_Should_Delete_Old_Published_Messages()
        {
            ...
            await ctx.Database.ExecuteSqlRawAsync(
                "UPDATE maintenance.OutboxMessages SET PublishedAt = DATEADD(DAY, -10, GETUTCDATE()) WHERE Id = {0}",
                msg.Id);
            ctx.ChangeTracker.Clear();
