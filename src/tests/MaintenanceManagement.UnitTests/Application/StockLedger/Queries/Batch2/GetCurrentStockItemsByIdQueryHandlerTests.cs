...
namespace MaintenanceManagement.UnitTests.Application.StockLedger.Queries.Batch2
{
    public sealed class GetCurrentStockItemsByIdQueryHandlerTests
    {
        ...
        {
            _mockQueryRepo
                .Setup(r => r.GetStockItemCodes(It.IsAny<string>(), It.IsAny<int>()))
        ...
        {
            SetupHappyPath(new List<StockItemCodeDto> { new() { ItemCode = "I01" } });
        ...
        public async Task Handle_CallsRepositoryOnce()
        {
            ...
            await CreateSut().Handle(
                new GetCurrentStockItemsByIdQuery { OldUnitcode = "U01", DepartmentId = 1 },
                CancellationToken.None);
            _mockQueryRepo.Verify(r => r.GetStockItemCodes("U01", 1), Times.Once);
        }
