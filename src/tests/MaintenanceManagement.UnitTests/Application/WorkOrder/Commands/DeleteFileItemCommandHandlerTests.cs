...
{
    public sealed class DeleteFileItemCommandHandlerTests
    {
        ...
        private DeleteFileItemCommandHandler CreateSut() =>
        ...
        private void SetupMocks(string baseDirectory = "Uploads", bool deleteResult = true)
        {
            ...
            _mockCommandRepo.Setup(r => r.GetBaseDirectoryItemAsync()).ReturnsAsync(baseDirectory);
            _mockFileService.Setup(f => f.DeleteFileAsync(It.IsAny<string>())).ReturnsAsync(deleteResult);
            _mockCommandRepo.Setup(r => r.DeleteItemImageAsync(It.IsAny<string>())).ReturnsAsync(true);
        }
        ...
        public async Task Handle_EmptyBaseDirectory_ReturnsFailure()
        {
            ...
            var result = await CreateSut().Handle(
                new DeleteFileItemCommand { Image = "test.jpg" }, CancellationToken.None);
        ...
        [Fact]
        public async Task Handle_CallsDeleteItemImageAsyncOnce()
        {
            SetupMocks();
            ...
            _mockCommandRepo.Verify(r => r.DeleteItemImageAsync(It.IsAny<string>()), Times.Once);
        }
    }
}
