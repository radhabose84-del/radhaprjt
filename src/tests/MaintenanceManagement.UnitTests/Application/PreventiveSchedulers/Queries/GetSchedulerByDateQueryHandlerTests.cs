...
namespace MaintenanceManagement.UnitTests.Application.PreventiveSchedulers.Queries.BatchD
{
    public sealed class GetSchedulerByDateQueryHandlerTests
    {
        ...
        private GetSchedulerByDateQueryHandler CreateSut()
        {
            ...
            _mockMapper.Setup(m => m.Map<List<SchedulerByDateDto>>(It.IsAny<object>()))
                .Returns(new List<SchedulerByDateDto>());
        ...
        {
            _mockQuery.Setup(q => q.GetAbstractSchedulerByDate(It.IsAny<int>()))
                .ReturnsAsync((IEnumerable<dynamic>)new List<dynamic>());

            var result = await CreateSut().Handle(new GetSchedulerByDateQuery { DepartmentId = 1 }, CancellationToken.None);
