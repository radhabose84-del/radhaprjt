    ...
    public sealed class UpdateRoleCommandValidatorTests
    {
        private static MaxLengthProvider CreateMaxLengthProvider()
        {
            var options = new DbContextOptionsBuilder<ApplicationDbContext>()
                .UseInMemoryDatabase(databaseName: $"UserRoleUpdateDb_{Guid.NewGuid()}")
                .Options;
        ...
        private static UpdateRoleCommandValidator CreateValidator() =>
            new(CreateMaxLengthProvider());

        private static UpdateRoleCommand ValidCommand() =>
            new UpdateRoleCommand { Id = 1, RoleName = "Admin", Description = "Administrator role", IsActive = 1 };
