...
{
    public sealed class CreateDivisionCommandValidatorTests
    {
        private static MaxLengthProvider CreateMaxLengthProvider()
        {
            var options = new DbContextOptionsBuilder<ApplicationDbContext>()
                .UseInMemoryDatabase(databaseName: $"DivisionValidatorDb_{Guid.NewGuid()}")
                .Options;
        ...
        private static CreateDivisionCommandValidator CreateValidator()
        {
            var maxLengthProvider = CreateMaxLengthProvider();
            return new CreateDivisionCommandValidator(maxLengthProvider);
        }
        ...
        {
            var command = DivisionBuilders.ValidCreateCommand(name: name);
            var result = await CreateValidator().TestValidateAsync(command);
        ...
        {
            var command = DivisionBuilders.ValidCreateCommand(shortName: shortName);
            var result = await CreateValidator().TestValidateAsync(command);
        ...
        {
            var command = DivisionBuilders.ValidCreateCommand(name: new string('A', 101));
            var result = await CreateValidator().TestValidateAsync(command);
            result.ShouldHaveValidationErrorFor(x => x.Name);
        }
        ...
        {
            var command = DivisionBuilders.ValidCreateCommand(shortName: new string('A', 51));
            var result = await CreateValidator().TestValidateAsync(command);
            result.ShouldHaveValidationErrorFor(x => x.ShortName);
        }

        [Fact]
        public async Task Validate_NameWithinMaxLength_PassesValidation()
        {
            var command = DivisionBuilders.ValidCreateCommand(name: new string('A', 100));
            var result = await CreateValidator().TestValidateAsync(command);
        ...
        {
            var command = DivisionBuilders.ValidCreateCommand(shortName: new string('A', 50));
            var result = await CreateValidator().TestValidateAsync(command);
        ...
        {
            var command = DivisionBuilders.ValidCreateCommand(companyId: 0);
            var result = await CreateValidator().TestValidateAsync(command);
        ...
        {
            var command = DivisionBuilders.ValidCreateCommand(companyId: -1);
            var result = await CreateValidator().TestValidateAsync(command);
            result.ShouldHaveValidationErrorFor(x => x.CompanyId);
        }
