using AutoMapper;
using Contracts.Interfaces;
using MediatR;
using UserManagement.Application.Common.Interfaces.ICompany;
using UserManagement.Application.Companies.Queries.GetCompanies;
using UserManagement.Application.Companies.Queries.GetCompanyAutoComplete;
using UserManagement.Domain.Events;

namespace UserManagement.UnitTests.Application.Company.Queries
{
    public sealed class GetCompanyAutoCompleteQueryHandlerTests
    {
        private readonly Mock<ICompanyQueryRepository> _mockQueryRepo = new(MockBehavior.Strict);
        private readonly Mock<IMapper> _mockMapper = new(MockBehavior.Loose);
        private readonly Mock<IMediator> _mockMediator = new(MockBehavior.Loose);
        private readonly Mock<IIPAddressService> _mockIpService = new(MockBehavior.Loose);

        private GetCompanyAutoCompleteQueryHandler CreateSut() =>
            new(_mockQueryRepo.Object, _mockMapper.Object, _mockMediator.Object, _mockIpService.Object);

        [Fact]
        public async Task Handle_SuperAdmin_ReturnsAdminResults()
        {
            _mockIpService.Setup(s => s.GetGroupCode()).Returns("SUPER_ADMIN");

            var entities = new List<UserManagement.Domain.Entities.Company> { new() { Id = 1 } };
            var dtoList = new List<CompanyAutoCompleteDTO> { new() };

            _mockQueryRepo
                .Setup(r => r.GetCompany_SuperAdmin(It.IsAny<string?>()))
                .ReturnsAsync(entities);

            _mockMapper
                .Setup(m => m.Map<List<CompanyAutoCompleteDTO>>(entities))
                .Returns(dtoList);

            var result = await CreateSut().Handle(
                new GetCompanyAutoCompleteQuery { SearchPattern = "test" },
                CancellationToken.None);

            result.Should().HaveCount(1);
        }

        [Fact]
        public async Task Handle_RegularUser_CallsGetCompanyWithUserId()
        {
            _mockIpService.Setup(s => s.GetGroupCode()).Returns("USER");
            _mockIpService.Setup(s => s.GetUserId()).Returns(42);

            var entities = new List<UserManagement.Domain.Entities.Company> { new() { Id = 1 } };
            var dtoList = new List<CompanyAutoCompleteDTO> { new() };

            _mockQueryRepo
                .Setup(r => r.GetCompany(42, It.IsAny<string?>()))
                .ReturnsAsync(entities);

            _mockMapper
                .Setup(m => m.Map<List<CompanyAutoCompleteDTO>>(entities))
                .Returns(dtoList);

            _mockMediator
                .Setup(m => m.Publish(It.IsAny<AuditLogsDomainEvent>(), It.IsAny<CancellationToken>()))
                .Returns(Task.CompletedTask);

            var result = await CreateSut().Handle(
                new GetCompanyAutoCompleteQuery { SearchPattern = "test" },
                CancellationToken.None);

            result.Should().HaveCount(1);
        }
    }
}
