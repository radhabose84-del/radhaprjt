...
{
    public sealed class DeleteAccessPolicyCommandHandlerTests
    {
        private readonly Mock<IAccessPolicyCommandRepository> _mockCommandRepo = new(MockBehavior.Strict);
        private readonly Mock<IMediator>                      _mockMediator    = new(MockBehavior.Loose);

        private DeleteAccessPolicyCommandHandler CreateSut() =>
        ...
        public async Task Handle_ExistingId_ReturnsTrue()
        {
            ...
            var result = await CreateSut().Handle(new DeleteAccessPolicyCommand(1), CancellationToken.None);
        ...
        public async Task Handle_ExistingId_PublishesAuditEvent()
        {
            ...
            await CreateSut().Handle(new DeleteAccessPolicyCommand(1), CancellationToken.None);

            _mockMediator.Verify(
                m => m.Publish(
                    It.Is<AuditLogsDomainEvent>(e =>
                        e.ActionDetail == "SoftDelete" &&
                        e.ActionCode   == "ACCESSPOLICY_DELETE"),
        ...
        public async Task Handle_NonExistentId_ThrowsExceptionRules()
        {
            ...
            Func<Task> act = async () =>
                await CreateSut().Handle(new DeleteAccessPolicyCommand(99), CancellationToken.None);
        ...
        public async Task Handle_NonExistentId_DoesNotPublishAuditEvent()
        {
            ...
            try { await CreateSut().Handle(new DeleteAccessPolicyCommand(99), CancellationToken.None); }
            catch { /* expected */ }
