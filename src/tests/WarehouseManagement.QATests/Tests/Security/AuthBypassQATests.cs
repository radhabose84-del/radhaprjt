...
namespace WarehouseManagement.QATests.Tests.Security;

// ─────────────────────────────────────────────────────────────────────────────
// Phase 3 — Security: authentication-bypass checks (WarehouseManagement surface).
//
...
public sealed class AuthBypassQATests
{
    ...
    private const string ProtectedGet  = "/api/WarehouseMaster?PageNumber=1&PageSize=5";
    private const string ProtectedPost = "/api/WarehouseMaster";
    ...
    // CONTROL — a genuine token IS accepted. WarehouseMaster GetAll is tolerant 200/404 on the clone.
    [Fact, TestPriority(1)]
    public async Task TC001_ValidToken_Returns200Or404()
    {
    ...
    public async Task TC007_Post_NoToken_Returns401()
    {
        ...
        {
            warehouseName = "Sec Bypass Attempt",
            unitId = 1,
            warehouseTypeId = 1,
            departmentId = 1,
            storageTypeId = 1,
