namespace WarehouseManagement.QATests.Tests.BinMaster;

// ─────────────────────────────────────────────────────────────────────────────
// BinMaster — live-server QA suite (negatives ACTIVE; create-happy + lifecycle SKIPPED).
//
// Contract verified against source (2026-06-16):
//   POST   /BinMaster/create        CreateBinMasterCommand { binName?, warehouseId(req), rackId?,
//                                       binCapacity(req,>0), capacityUOMId(req) }
//   PUT    /BinMaster/update         UpdateBinMasterCommand
//   DELETE /BinMaster?id={id}        (id bound from QUERY — action param `int id`)
//   GET    /BinMaster?PageNumber=&PageSize=&SearchTerm=
//   GET    /BinMaster/{id}           (GetById THROWS 500 when missing → tolerate 200/404/500)
//   GET    /BinMaster/by-name?name=&warehouseId=&rackId=
//
// ⚠️ BinMasterController = [Route("[controller]")] — NO /api prefix. Routes are case-insensitive
//    so "/BinMaster" is used throughout.
//
// Why create-happy + lifecycle are SKIPPED:
//   BinMaster.warehouseId is a same-module WarehouseMaster FK; the QA clone has 0 warehouses for
//   testsales (WarehouseMaster + RackMaster create are themselves blocked). Without a resolvable
//   warehouse id no valid bin can be created. Attribute-level [Fact(Skip=...)]. Negatives (empty
//   body / missing warehouseId / no-auth), smoke GetAll and by-name reachability remain ACTIVE.
// ─────────────────────────────────────────────────────────────────────────────

[Collection("BinMasterCollection")]
[TestCaseOrderer("Shared.QAInfrastructure.Infrastructure.PriorityOrderer", "Shared.QAInfrastructure")]
public sealed class BinMasterQATests
{
    private readonly QAServerFixture _f;
    private const string BaseRoute = "/BinMaster";

    public BinMasterQATests(QAServerFixture fixture) => _f = fixture;
    ...
    public async Task TC001_Create_HappyPath_Returns200_And_CapturesId()
    {
        ...
        {
            binName = "QA Bin " + _f.EntityCode[..8],
            warehouseId,
            binCapacity = 100m,
            capacityUOMId = 1
        });
    ...
    {
        // BUG (live): GetBinMasterByIdQuery throws when the id is missing → 500 instead of a clean
    ...
    // ─────────────────────────────────────────────────────────────────────────
    // SECTION 4 — AUTOCOMPLETE  (params: name, warehouseId, rackId)
    // ─────────────────────────────────────────────────────────────────────────

    [Fact, TestPriority(40)]
    public async Task TC040_AutoComplete_WithName_Returns200Or404()
    {
        var resp = await _f.Client.GetAsync($"{BaseRoute}/by-name?name=QA&warehouseId=0&rackId=0");
        ((int)resp.StatusCode).Should().BeOneOf(200, 404);
    }
    ...
    [Fact(Skip = "needs seeded data: a created BinMaster id (TC001 blocked on a WarehouseMaster id)"), TestPriority(50)]
    ...
    [Fact(Skip = "needs seeded data: a created BinMaster id (TC001 blocked on a WarehouseMaster id)"), TestPriority(92)]
