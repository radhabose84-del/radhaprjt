namespace ProjectManagement.QATests.Tests.MiscMaster;

// ─────────────────────────────────────────────────────────────────────────────
// MiscMaster (ProjectManagement) — live-server QA suite (CRUD lifecycle + negatives).
//
// Contract verified against source (2026-06-16):
//   POST   /api/project/MiscMaster            { miscTypeId, code, description }   (all 3 REQUIRED)
//   PUT    /api/project/MiscMaster            { id, miscTypeId, code, description, sortOrder, isActive(byte 0/1) }
//   DELETE /api/project/MiscMaster/{id}       (id bound from ROUTE, not query)
//   GET    /api/project/MiscMaster?PageNumber=&PageSize=&SearchTerm=
//   GET    /api/project/MiscMaster/{id}       (returns 200 — no 404 guard; raw query result wrapped)
//   GET    /api/project/MiscMaster/by-name?name=&MiscTypeCode=
//
// Key facts that shaped assertions:
//   • Code is alphanumeric (^[A-Za-z0-9]+$), max 50, unique per miscTypeId. Description REQUIRED (max 250).
//   • MiscTypeId is a same-module FK (Project.MiscTypeMaster) — resolved at runtime via
//       FirstIdAsync on /api/project/MiscTypeMaster (fallback 1).
//   • Create returns a BARE GetMiscMasterDto → data is an OBJECT carrying the new id.
//   • Update command DOES carry miscTypeId + code (per source — mutable, unlike Sales MiscMaster).
//   • Delete controller does NOT 404-guard — non-existent delete may still return 200 (tolerant).
// ─────────────────────────────────────────────────────────────────────────────

[Collection("ProjectMiscMasterCollection")]
[TestCaseOrderer("Shared.QAInfrastructure.Infrastructure.PriorityOrderer", "Shared.QAInfrastructure")]
public sealed class MiscMasterQATests
{
    private readonly QAServerFixture _f;
    private const string BaseRoute = "/api/project/MiscMaster";
    private const string MiscTypeRoute = "/api/project/MiscTypeMaster";

    private const string TestDescription = "QA Test Project Misc Master";
    ...
    public async Task TC006_Create_CodeTooLong_Returns400()
    {
        ...
        {
            miscTypeId = await ResolveMiscTypeIdAsync(),
            code = new string('A', 101), // exceeds max 50
            description = TestDescription
        });
    ...
    // ─────────────────────────────────────────────────────────────────────────
    // SECTION 3 — GET BY ID  (controller has NO 404 guard → 200)
    // ─────────────────────────────────────────────────────────────────────────
    ...
    {
        var resp = await _f.Client.GetAsync($"{BaseRoute}/by-name?MiscTypeCode=QA");
        await QAHelper.AssertOkAsync(resp);
    }
    ...
    // ─────────────────────────────────────────────────────────────────────────
    // SECTION 5 — UPDATE  (command carries miscTypeId + code per source)
    // ─────────────────────────────────────────────────────────────────────────
    ...
    public async Task TC050_Update_HappyPath_Returns200()
    {
        var resp = await _f.Client.PutAsJsonAsync(BaseRoute, new
        {
            ...
            miscTypeId = _miscTypeId,
            code = _createdCode,
            description = "QA Updated Project Misc Master",
            sortOrder = 1,
            isActive = (byte)1
        });
    ...
    public async Task TC090_Delete_NonExistentId_Returns200Or400Or404()
    {
        // Delete controller does NOT 404-guard the handler result → tolerant.
