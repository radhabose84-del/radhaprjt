...
namespace ProjectManagement.QATests.Tests.Security;

// ─────────────────────────────────────────────────────────────────────────────
// Phase 3 — Security: authentication-bypass checks (ProjectManagement surface).
//
...
public sealed class AuthBypassQATests
{
    ...
    private const string ProtectedGet  = "/api/ProjectMaster?PageNumber=1&PageSize=5";
    private const string ProtectedPost = "/api/ProjectMaster";
    ...
    public async Task TC007_Post_NoToken_Returns401()
    {
        ...
        {
            projectName = "Sec Bypass Attempt",
            projectTypeId = 1
        });
        resp.StatusCode.Should().Be(HttpStatusCode.Unauthorized);
    }
