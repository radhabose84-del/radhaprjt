...
namespace FixedAssetManagement.IntegrationTests.Repositories.MiscTypeMaster
{
    ...
    public sealed class MiscTypeMasterCommandRepositoryTests
    {
        ...
        private MiscTypeMasterCommandRepository CreateRepository(FAM.Infrastructure.Data.ApplicationDbContext ctx) =>
            new(ctx);

        private static FAM.Domain.Entities.MiscTypeMaster BuildEntity(
            string code = "FAM_TYPE",
            string description = "Asset Type") =>
        ...
        public async Task CreateAsync_Should_Persist_Fields_Correctly()
        {
            ...
            var result = await CreateRepository(ctx).CreateAsync(BuildEntity("FAM_TYPE", "Asset Type"));
            ctx.ChangeTracker.Clear();

            var saved = await ctx.MiscTypeMaster.FirstOrDefaultAsync(x => x.Id == result.Id);

            saved.Should().NotBeNull();
            saved.MiscTypeCode.Should().Be("FAM_TYPE");
            saved.Description.Should().Be("Asset Type");
        ...
        public async Task UpdateAsync_Should_Persist_Changes()
        {
            ...
            var toUpdate = new FAM.Domain.Entities.MiscTypeMaster
            {
                Id = created.Id,
