...
namespace FixedAssetManagement.IntegrationTests.Repositories.MiscTypeMaster
{
    ...
    public sealed class MiscTypeMasterQueryRepositoryTests
    {
        ...
        private async Task<int> SeedEntityAsync(string code = "FAM_TYPE", string description = "Asset Type")
        {
            await using var ctx = _fixture.CreateFreshDbContext();
            var repo = new MiscTypeMasterCommandRepository(ctx);
            var entity = new FAM.Domain.Entities.MiscTypeMaster
            {
                MiscTypeCode = code,
        ...
        {
            await ClearTableAsync();
            await SeedEntityAsync("FAM_TYPE", "Asset Type");

            var (items, _) = await CreateQueryRepo().GetAllMiscTypeMasterAsync(1, 10, null);

            items[0].MiscTypeCode.Should().Be("FAM_TYPE");
            items[0].Description.Should().Be("Asset Type");
        }
        ...
        public async Task GetAllMiscTypeMasterAsync_Should_Exclude_SoftDeleted()
        {
            ...
            await using var ctx = _fixture.CreateFreshDbContext();
            var toDelete = new FAM.Domain.Entities.MiscTypeMaster
            {
        ...
        {
            await ClearTableAsync();
            var id = await SeedEntityAsync("FAM_TYPE", "Asset Type");

            var result = await CreateQueryRepo().GetByIdAsync(id);

            result.Should().NotBeNull();
            result.Id.Should().Be(id);
            result.MiscTypeCode.Should().Be("FAM_TYPE");
        }

        // --- GET BY CODE ---

        [Fact]
        public async Task GetByMiscTypeMasterCodeAsync_Should_Return_Record_When_Exists()
        {
            await ClearTableAsync();
            await SeedEntityAsync("FAM_TYPE", "Asset Type");

            var result = await CreateQueryRepo().GetByMiscTypeMasterCodeAsync("FAM_TYPE");
        ...
        [Fact]
        public async Task GetByMiscTypeMasterCodeAsync_Should_Return_Null_When_NotFound()
        {
            await ClearTableAsync();

            var result = await CreateQueryRepo().GetByMiscTypeMasterCodeAsync("NONEXISTENT");

            result.Should().BeNull();
        }

        [Fact]
        public async Task GetByMiscTypeMasterCodeAsync_Should_Exclude_Self_When_Id_Provided()
        {
            await ClearTableAsync();
            var id = await SeedEntityAsync("FAM_TYPE");

            var result = await CreateQueryRepo().GetByMiscTypeMasterCodeAsync("FAM_TYPE", id);
        ...
        public async Task GetMiscTypeMaster_Should_Return_Matching_Records()
        {
            await ClearTableAsync();
            await SeedEntityAsync("FAM_TYPE", "Asset Type");
            await SeedEntityAsync("FAM_STATUS", "Asset Status");
            await SeedEntityAsync("OTHER", "Other");

            var results = await CreateQueryRepo().GetMiscTypeMaster("FAM");
