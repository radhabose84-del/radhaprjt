...
namespace ProjectManagement.IntegrationTests.Repositories.MiscTypeMaster
{
    ...
    public sealed class MiscTypeMasterQueryRepositoryTests
    {
        ...
        private async Task<int> SeedEntityAsync(string code = "PROJ_TYPE", string description = "Project Type")
        {
        ...
        {
            await ClearTableAsync();
            await SeedEntityAsync("STATUS", "Status Type");

            var (items, _) = await CreateQueryRepo().GetAllMiscTypeMasterAsync(1, 10, null);

            items[0].MiscTypeCode.Should().Be("STATUS");
            items[0].Description.Should().Be("Status Type");
        }
        ...
        public async Task GetAllMiscTypeMasterAsync_Should_Exclude_SoftDeleted()
        {
            ...
            await using var ctx = _fixture.CreateFreshDbContext();
            var toDelete = new ProjectManagement.Domain.Entities.MiscTypeMaster
            {
        ...
        {
            await ClearTableAsync();
            var id = await SeedEntityAsync("PROJ_TYPE", "Project Type");

            var result = await CreateQueryRepo().GetByIdAsync(id);

            result.Should().NotBeNull();
            result.Id.Should().Be(id);
            result.MiscTypeCode.Should().Be("PROJ_TYPE");
        }
        ...
        {
            await ClearTableAsync();
            await SeedEntityAsync("PROJ_TYPE");

            var exists = await CreateQueryRepo().AlreadyExistsAsync("PROJ_TYPE");
        ...
        {
            await ClearTableAsync();
            var id = await SeedEntityAsync("PROJ_TYPE");
        ...
        public async Task AlreadyExistsAsync_Should_Exclude_Self_When_Id_Provided()
        {
            ...
            var exists = await CreateQueryRepo().AlreadyExistsAsync("PROJ_TYPE", id);
        ...
        {
            await ClearTableAsync();
            await SeedEntityAsync("PROJ_TYPE", "Project Type");
            await SeedEntityAsync("PROJ_STATUS", "Project Status");
            await SeedEntityAsync("OTHER", "Other");

            var results = await CreateQueryRepo().GetMiscTypeMaster("PROJ");
