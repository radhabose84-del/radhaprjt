...
namespace ProjectManagement.IntegrationTests.Repositories.MiscTypeMaster
{
    ...
    public sealed class MiscTypeMasterCommandRepositoryTests
    {
        ...
        private MiscTypeMasterCommandRepository CreateRepository(ProjectManagement.Infrastructure.Data.ApplicationDbContext ctx) =>
            new(ctx);

        private static ProjectManagement.Domain.Entities.MiscTypeMaster BuildEntity(
            string code = "PROJ_TYPE",
            string description = "Project Type") =>
        ...
        public async Task CreateAsync_Should_Persist_Fields_Correctly()
        {
            ...
            var result = await CreateRepository(ctx).CreateAsync(BuildEntity("PROJ_TYPE", "Project Type"));
            ctx.ChangeTracker.Clear();

            var saved = await ctx.MiscTypeMaster.FirstOrDefaultAsync(x => x.Id == result.Id);

            saved.Should().NotBeNull();
            saved.MiscTypeCode.Should().Be("PROJ_TYPE");
            saved.Description.Should().Be("Project Type");
        ...
        public async Task UpdateAsync_Should_Persist_Changes()
        {
            ...
            var toUpdate = new ProjectManagement.Domain.Entities.MiscTypeMaster
            {
                Id = created.Id,
            ...
            updated.Description.Should().Be("Updated Name");
        }
