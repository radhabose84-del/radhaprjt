...
namespace SalesManagement.QATests.Tests.Security;

// ─────────────────────────────────────────────────────────────────────────────
// Phase 3 — Security: authentication-bypass checks (SalesManagement surface).
//
...
// (documented live behavior, same middleware as UserManagement). Un-skip when the
// middleware is fixed to return 401.
// ─────────────────────────────────────────────────────────────────────────────
...
public sealed class AuthBypassQATests
{
    ...
    private const string ProtectedGet  = "/api/SalesOrganisation?PageNumber=1&PageSize=5";
    private const string ProtectedPost = "/api/SalesOrganisation";
    ...
    public async Task TC007_Post_NoToken_Returns401()
    {
        ...
        {
            salesOrganisationCode = "SEC01",
            salesOrganisationName = "Sec Bypass Attempt",
            companyId = 1
        });
        resp.StatusCode.Should().Be(HttpStatusCode.Unauthorized);
    }
