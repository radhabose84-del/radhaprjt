namespace SalesManagement.QATests.Tests.SalesOrganisation;

// ─────────────────────────────────────────────────────────────────────────────
// SalesOrganisation — live-server QA suite (CRUD lifecycle + negatives).
//
// Contract verified against source (2026-06-09):
//   POST   /api/SalesOrganisation            { salesOrganisationCode, salesOrganisationName, companyId, description? }
//   PUT    /api/SalesOrganisation            { id, salesOrganisationName, companyId, description?, isActive }
//   DELETE /api/SalesOrganisation?id={id}    (id bound from QUERY, not route)
//   GET    /api/SalesOrganisation?PageNumber=&PageSize=&SearchTerm=
//   GET    /api/SalesOrganisation/{id}       (returns 200 + data:null when not found — NO 404 guard)
//   GET    /api/SalesOrganisation/by-name?term=
//
// Key facts that shaped assertions:
//   • Code is alphanumeric (^[A-Za-z0-9]+$) — digits allowed, spaces/specials rejected. Immutable on update.
//   • CompanyId is a cross-module FK validated via ICompanyLookup.CompanyExistsAsync — uses companyId = 1 (seeded).
//   • Reads are NOT company-scoped (WHERE IsDeleted = 0 only) → created rows are visible in GetAll/GetById.
//   • Delete validator: NotEmpty (id!=0) → NotFound (must exist) → SoftDelete (no dependent SalesOffice/SalesSegment).
// ─────────────────────────────────────────────────────────────────────────────

[Collection("SalesOrganisationCollection")]
[TestCaseOrderer("Shared.QAInfrastructure.Infrastructure.PriorityOrderer", "Shared.QAInfrastructure")]
public sealed class SalesOrganisationQATests
{
    private readonly QAServerFixture _f;
    private const string BaseRoute = "/api/SalesOrganisation";

    // A real, seeded company id. CompanyExistsAsync checks the (un-scoped) company lookup,
    // so id 1 is the conventional seed. Live reconciliation may adjust if the clone differs.
    private const int ValidCompanyId = 1;

    private const string TestName = "QA Test Sales Organisation";
    ...
    public SalesOrganisationQATests(QAServerFixture fixture) => _f = fixture;

    // Run-unique, alphanumeric (Q + reversed-tick digits), sliced to 10 chars.
    ...
    public async Task TC001_Create_HappyPath_Returns200_And_CapturesId()
    {
        ...
        {
            salesOrganisationCode = _createdCode,
            salesOrganisationName = TestName,
    ...
    public async Task TC002_Create_NoAuthToken_Returns401()
    {
        ...
        {
            salesOrganisationCode = "NOAUTH01",
            salesOrganisationName = "No Auth Org",
            companyId = ValidCompanyId
        });
    ...
    [Fact, TestPriority(5)]
    public async Task TC005_Create_CompanyIdMissing_Returns400()
    {
        // CompanyId NotEmpty → default 0 fails validation.
        var resp = await _f.Client.PostAsJsonAsync(BaseRoute, new
        {
            salesOrganisationCode = NewCode(),
            salesOrganisationName = TestName,
    ...
    public async Task TC006_Create_CodeTooLong_Returns400()
    {
        var resp = await _f.Client.PostAsJsonAsync(BaseRoute, new
        {
            salesOrganisationCode = new string('A', 101), // exceeds any code max
            salesOrganisationName = TestName,
            companyId = ValidCompanyId
        });
    ...
    public async Task TC007_Create_NameTooLong_Returns400()
    {
        ...
        {
            salesOrganisationCode = NewCode(),
            salesOrganisationName = new string('A', 101), // exceeds name max (100)
            companyId = ValidCompanyId
        });
    ...
    public async Task TC011_Create_NonExistentCompany_Returns400()
    {
        // CompanyExistsAsync false → FK validation fails.
        var resp = await _f.Client.PostAsJsonAsync(BaseRoute, new
        {
            salesOrganisationCode = NewCode(),
            salesOrganisationName = TestName,
    ...
    public async Task TC030_GetById_ValidId_Returns200_WithCorrectCode()
    {
        ...
        await QAHelper.AssertOkAsync(resp);
        var doc = await QAHelper.ParseAsync(resp);
        doc.RootElement.GetProperty("data").GetProperty("salesOrganisationCode").GetString()
    ...
    public async Task TC050_Update_HappyPath_Returns200()
    {
        ...
        {
            id = _f.CreatedId,
            salesOrganisationName = "QA Updated Organisation",
            companyId = ValidCompanyId,
