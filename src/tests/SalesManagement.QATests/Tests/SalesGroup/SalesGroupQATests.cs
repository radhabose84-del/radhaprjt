namespace SalesManagement.QATests.Tests.SalesGroup;

// ─────────────────────────────────────────────────────────────────────────────
// SalesGroup — live-server QA suite (CRUD lifecycle + negatives).
//
...
//   PUT    /api/SalesGroup            { id, salesGroupName, salesOfficeId, responsibleManager?, productCategoryId?, regionTerritory?, isActive }
//   DELETE /api/SalesGroup?id={id}    (id bound from QUERY, not route — DeleteSalesGroup(int id))
//   GET    /api/SalesGroup?PageNumber=&PageSize=&SearchTerm=
//   GET    /api/SalesGroup/{id}       (returns 200 + data:null when not found — NO 404 guard)
//   GET    /api/SalesGroup/by-name?term=
//
// Key facts that shaped assertions:
//   • NO unique code field. Uniqueness is on (SalesGroupName + SalesOfficeId) composite (AlreadyExistsAsync).
//     → run-unique names avoid "already exists" 400s; NO alphanumeric/duplicate-code tests.
//   • salesOfficeId is a SAME-module FK validated via SalesOfficeExistsAsync — resolved at runtime
//     from /api/SalesOffice GETALL (fallback 1).
//   • productCategoryId / responsibleManager / regionTerritory are optional — omitted in happy path.
...
[Collection("SalesGroupCollection")]
[TestCaseOrderer("Shared.QAInfrastructure.Infrastructure.PriorityOrderer", "Shared.QAInfrastructure")]
public sealed class SalesGroupQATests
{
    private readonly QAServerFixture _f;
    private const string BaseRoute = "/api/SalesGroup";

    private const string TestName = "QA Test Sales Group";

    // Run-unique name captured at create; reused by GetAll-search.
    private static string _createdName = string.Empty;

    // Same-module FK (SalesOffice) resolved once at create.
    private static int _salesOfficeId;

    public SalesGroupQATests(QAServerFixture fixture) => _f = fixture;

    // Run-unique name: base + reversed-tick digits slice → re-runnable without DB reset.
    private string NewName() => $"{TestName} {_f.EntityCode[1..7]}";

    private async Task<int> ResolveSalesOfficeIdAsync()
    {
        if (_salesOfficeId > 0) return _salesOfficeId;
        var id = await QAHelper.FirstIdAsync(_f.Client, "/api/SalesOffice");
        _salesOfficeId = id > 0 ? id : 1;
        return _salesOfficeId;
    }
    ...
    public async Task TC001_Create_HappyPath_Returns200_And_CapturesId()
    {
        ...
        var salesOfficeId = await ResolveSalesOfficeIdAsync();

        var resp = await _f.Client.PostAsJsonAsync(BaseRoute, new
        {
            salesGroupName = _createdName,
            salesOfficeId,
    ...
    [Fact, TestPriority(4)]
    public async Task TC004_Create_SalesOfficeIdMissing_Returns400()
    {
        // SalesOfficeId GreaterThan(0) → default 0 fails validation.
        var resp = await _f.Client.PostAsJsonAsync(BaseRoute, new
        {
            salesGroupName = NewName(),
            salesOfficeId = 0
        });
    ...
    public async Task TC005_Create_NameTooLong_Returns400()
    {
        ...
        var resp = await _f.Client.PostAsJsonAsync(BaseRoute, new
        {
            salesGroupName = new string('A', 101), // exceeds name max (100)
            salesOfficeId
        });
    ...
    [Fact, TestPriority(6)]
    public async Task TC006_Create_NonExistentSalesOffice_Returns400()
    {
        // SalesOfficeExistsAsync false → FK validation fails.
        var resp = await _f.Client.PostAsJsonAsync(BaseRoute, new
        {
            salesGroupName = NewName(),
    ...
    public async Task TC030_GetById_ValidId_Returns200_WithCorrectName()
    {
        ...
        await QAHelper.AssertOkAsync(resp);
        var doc = await QAHelper.ParseAsync(resp);
        doc.RootElement.GetProperty("data").GetProperty("salesGroupName").GetString()
    ...
    public async Task TC050_Update_HappyPath_Returns200()
    {
        ...
        var resp = await _f.Client.PutAsJsonAsync(BaseRoute, new
        {
            ...
            regionTerritory = "QA Updated Region",
            isActive = 1
        });
