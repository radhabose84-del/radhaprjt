namespace SalesManagement.QATests.Tests.MiscMaster;

// ─────────────────────────────────────────────────────────────────────────────
// MiscMaster — live-server QA suite (CRUD lifecycle + negatives).
//
// Contract verified against source (2026-06-15):
//   POST   /api/sales/MiscMaster             { miscTypeId, code, description }  (all 3 REQUIRED)
//   PUT    /api/sales/MiscMaster             { id, description, sortOrder, isActive } (code + miscTypeId immutable)
//   DELETE /api/sales/MiscMaster?id={id}     (id bound from QUERY, not route)
//   GET    /api/sales/MiscMaster?PageNumber=&PageSize=&SearchTerm=&MiscTypeId=
//   GET    /api/sales/MiscMaster/{id}        (returns 200 + data:null when not found — NO 404 guard)
//   GET    /api/sales/MiscMaster/by-name?term=&MiscTypeCode=
//
...
//   • MiscTypeId is a same-module FK (Sales.MiscTypeMaster) validated via MiscTypeExistsAsync.
//       → resolved at runtime via FirstIdAsync on /api/sales/MiscTypeMaster (fallback 1).
//   • AlreadyExists is COMPOSITE: uniqueness is (Code + MiscTypeId) — duplicate test reuses both.
//   • GetById data carries the misc code under "code" (not "miscTypeCode" — that is the parent type code).
//   • Delete validator: NotEmpty (id!=0) → NotFound (must exist).
// ─────────────────────────────────────────────────────────────────────────────

[Collection("SalesMiscMasterCollection")]
[TestCaseOrderer("Shared.QAInfrastructure.Infrastructure.PriorityOrderer", "Shared.QAInfrastructure")]
public sealed class MiscMasterQATests
{
    private readonly QAServerFixture _f;
    private const string BaseRoute = "/api/sales/MiscMaster";
    ...
    public async Task TC006_Create_CodeTooLong_Returns400()
    {
        ...
        {
            miscTypeId = await ResolveMiscTypeIdAsync(),
            code = new string('A', 101), // exceeds any code max
            description = TestDescription
        });
