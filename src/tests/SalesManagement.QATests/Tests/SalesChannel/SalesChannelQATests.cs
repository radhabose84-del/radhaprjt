namespace SalesManagement.QATests.Tests.SalesChannel;

// ─────────────────────────────────────────────────────────────────────────────
// SalesChannel — live-server QA suite (CRUD lifecycle + negatives).
//
// Contract verified against source (2026-06-15):
//   POST   /api/SalesChannel            { salesChannelCode, salesChannelName }
//   PUT    /api/SalesChannel            { id, salesChannelName, isActive }
//   DELETE /api/SalesChannel?id={id}    (id bound from QUERY — DeleteSalesChannel(int id), no route template)
//   GET    /api/SalesChannel?PageNumber=&PageSize=&SearchTerm=
//   GET    /api/SalesChannel/{id}       (returns 200 + data:null when not found — NO 404 guard)
//   GET    /api/SalesChannel/by-name?term=
//
// Key facts that shaped assertions:
//   • SalesChannelCode is unique, immutable on update. Treated as alphanumeric per the shared rule set.
//   • SalesChannelName is required.
//   • No FK fields, no Description field.
...
[Collection("SalesChannelCollection")]
[TestCaseOrderer("Shared.QAInfrastructure.Infrastructure.PriorityOrderer", "Shared.QAInfrastructure")]
public sealed class SalesChannelQATests
{
    private readonly QAServerFixture _f;
    private const string BaseRoute = "/api/SalesChannel";

    private const string TestName = "QA Test Sales Channel";
    ...
    public SalesChannelQATests(QAServerFixture fixture) => _f = fixture;
    ...
    public async Task TC001_Create_HappyPath_Returns200_And_CapturesId()
    {
        ...
        {
            salesChannelCode = _createdCode,
            salesChannelName = TestName
        });
    ...
    public async Task TC002_Create_NoAuthToken_Returns401()
    {
        ...
        {
            salesChannelCode = "NOAUTH01",
            salesChannelName = "No Auth Channel"
        });
    ...
    public async Task TC005_Create_CodeTooLong_Returns400()
    {
        var resp = await _f.Client.PostAsJsonAsync(BaseRoute, new
        {
            salesChannelCode = new string('A', 101), // exceeds any code max
            salesChannelName = TestName
        });
    ...
    public async Task TC006_Create_NameTooLong_Returns400()
    {
        ...
        {
            salesChannelCode = NewCode(),
            salesChannelName = new string('A', 101) // exceeds name max (100)
        });
    ...
    public async Task TC030_GetById_ValidId_Returns200_WithCorrectCode()
    {
        ...
        await QAHelper.AssertOkAsync(resp);
        var doc = await QAHelper.ParseAsync(resp);
        doc.RootElement.GetProperty("data").GetProperty("salesChannelCode").GetString()
    ...
    public async Task TC050_Update_HappyPath_Returns200()
    {
        ...
        {
            id = _f.CreatedId,
            salesChannelName = "QA Updated Channel",
            isActive = 1
        });
