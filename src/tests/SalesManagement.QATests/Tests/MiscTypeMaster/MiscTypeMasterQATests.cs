namespace SalesManagement.QATests.Tests.MiscTypeMaster;

// ─────────────────────────────────────────────────────────────────────────────
// MiscTypeMaster — live-server QA suite (CRUD lifecycle + negatives).
//
// Contract verified against source (2026-06-15):
//   POST   /api/sales/MiscTypeMaster          { miscTypeCode, description }   (description REQUIRED)
//   PUT    /api/sales/MiscTypeMaster          { id, description, isActive }   (code immutable)
//   DELETE /api/sales/MiscTypeMaster?id={id}  (id bound from QUERY, not route)
//   GET    /api/sales/MiscTypeMaster?PageNumber=&PageSize=&SearchTerm=
//   GET    /api/sales/MiscTypeMaster/{id}     (returns 200 + data:null when not found — NO 404 guard)
//   GET    /api/sales/MiscTypeMaster/by-name?term=
//
...
//   • MaxLength sourced from EF metadata (default 20 for code) → "longer than" message.
//   • No FK. AlreadyExists checks code uniqueness (IsDeleted=0).
//   • Delete validator: NotEmpty (id!=0) → NotFound (must exist).
// ─────────────────────────────────────────────────────────────────────────────

[Collection("SalesMiscTypeMasterCollection")]
[TestCaseOrderer("Shared.QAInfrastructure.Infrastructure.PriorityOrderer", "Shared.QAInfrastructure")]
public sealed class MiscTypeMasterQATests
{
    private readonly QAServerFixture _f;
    private const string BaseRoute = "/api/sales/MiscTypeMaster";

    private const string TestDescription = "QA Test Misc Type";
