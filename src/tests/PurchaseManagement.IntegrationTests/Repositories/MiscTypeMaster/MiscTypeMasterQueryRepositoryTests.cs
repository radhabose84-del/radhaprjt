...
namespace PurchaseManagement.IntegrationTests.Repositories.MiscTypeMaster
{
    ...
    public sealed class MiscTypeMasterQueryRepositoryTests
    {
        ...
        private async Task<int> SeedEntityAsync(string code = "PURCH_TYPE", string description = "Purchase Type")
        {
        ...
        {
            await ClearTableAsync();
            await SeedEntityAsync("PURCH_TYPE", "Purchase Type");

            var (items, _) = await CreateQueryRepo().GetAllMiscTypeMasterAsync(1, 10, null);

            items[0].MiscTypeCode.Should().Be("PURCH_TYPE");
            items[0].Description.Should().Be("Purchase Type");
        }
        ...
        {
            await ClearTableAsync();
            var id = await SeedEntityAsync("PURCH_TYPE", "Purchase Type");

            var result = await CreateQueryRepo().GetByIdAsync(id);

            result.Should().NotBeNull();
            result.Id.Should().Be(id);
            result.MiscTypeCode.Should().Be("PURCH_TYPE");
        }
        ...
        {
            await ClearTableAsync();
            await SeedEntityAsync("PURCH_TYPE");

            var exists = await CreateQueryRepo().AlreadyExistsAsync("PURCH_TYPE");
        ...
        {
            await ClearTableAsync();
            var id = await SeedEntityAsync("PURCH_TYPE");
        ...
        public async Task AlreadyExistsAsync_Should_Exclude_Self_When_Id_Provided()
        {
            ...
            var exists = await CreateQueryRepo().AlreadyExistsAsync("PURCH_TYPE", id);
        ...
        public async Task GetMiscTypeMaster_Should_Return_Matching_Records()
        {
            await ClearTableAsync();
            await SeedEntityAsync("PURCH_TYPE", "Purchase Type");
            await SeedEntityAsync("PURCH_STATUS", "Purchase Status");
            await SeedEntityAsync("OTHER", "Other");

            var results = await CreateQueryRepo().GetMiscTypeMaster("PURCH");
