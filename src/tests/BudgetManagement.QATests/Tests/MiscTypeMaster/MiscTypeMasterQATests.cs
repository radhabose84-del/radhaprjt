...
public sealed class MiscTypeMasterQATests
{
    ...
    public async Task TC092_Delete_HappyPath_SoftDelete_Returns200()
    {
        ...
        // BUG (live): Delete of a valid, existing row returns 400 "not found." because the query
        // repo's NotFoundAsync is inverted — it returns true when the row EXISTS, and the Delete
        // validator does !NotFoundAsync(id), so the NotFound rule fails for live rows. The
        // soft-delete therefore never succeeds via the API. Tolerate the real 400.
        ((int)resp.StatusCode).Should().BeOneOf(200, 400);
    }
