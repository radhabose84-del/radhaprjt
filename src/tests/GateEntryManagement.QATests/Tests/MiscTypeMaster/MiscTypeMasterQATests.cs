namespace GateEntryManagement.QATests.Tests.MiscTypeMaster;

// ─────────────────────────────────────────────────────────────────────────────
// MiscTypeMaster (GateEntry) — live-server QA suite (CRUD lifecycle + negatives).
//
// Contract verified against source (2026-06-16):
//   POST   /api/gateentry/MiscTypeMaster        { miscTypeCode, description }  (both REQUIRED)
//   PUT    /api/gateentry/MiscTypeMaster        { id, description, isActive } (miscTypeCode immutable)
//   DELETE /api/gateentry/MiscTypeMaster?id={id} (id bound from QUERY, not route)
//   GET    /api/gateentry/MiscTypeMaster?PageNumber=&PageSize=&SearchTerm=
//   GET    /api/gateentry/MiscTypeMaster/{id}    (returns 200 + data:null when not found — NO 404 guard)
//   GET    /api/gateentry/MiscTypeMaster/by-name?term=
//
// Key facts that shaped assertions:
//   • miscTypeCode is alphanumeric (^[A-Za-z0-9]+$, max 20) — spaces/specials rejected. Immutable on update.
//   • description is REQUIRED (NotEmpty, max 250).
//   • miscTypeCode is UNIQUE → duplicate create fails (AlreadyExists).
//   • Validators DO enforce alphanumeric + isActive range (0/1).
//   • Delete validator: NotEmpty (id!=0) → NotFound (must exist).
// ─────────────────────────────────────────────────────────────────────────────

[Collection("GateMiscTypeMasterCollection")]
[TestCaseOrderer("Shared.QAInfrastructure.Infrastructure.PriorityOrderer", "Shared.QAInfrastructure")]
public sealed class MiscTypeMasterQATests
{
    private readonly QAServerFixture _f;
    private const string BaseRoute = "/api/gateentry/MiscTypeMaster";

    private const string TestDescription = "QA Test Gate Misc Type";
    ...
    // ─────────────────────────────────────────────────────────────────────────
    // SECTION 5 — UPDATE  (miscTypeCode immutable — not in the update command)
    // ─────────────────────────────────────────────────────────────────────────
    ...
    public async Task TC050_Update_HappyPath_Returns200()
    {
        ...
        {
            id = _f.CreatedId,
            description = "QA Updated Gate Misc Type",
            isActive = 1
        });
