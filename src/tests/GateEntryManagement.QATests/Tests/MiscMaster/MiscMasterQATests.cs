namespace GateEntryManagement.QATests.Tests.MiscMaster;

// ─────────────────────────────────────────────────────────────────────────────
// MiscMaster (GateEntry) — live-server QA suite (CRUD lifecycle + negatives).
//
// Contract verified against source (2026-06-16):
//   POST   /api/gateentry/MiscMaster            { miscTypeId, code, description }  (all 3 REQUIRED)
//   PUT    /api/gateentry/MiscMaster            { id, description, sortOrder, isActive } (code + miscTypeId immutable)
//   DELETE /api/gateentry/MiscMaster?id={id}    (id bound from QUERY, not route)
//   GET    /api/gateentry/MiscMaster?PageNumber=&PageSize=&SearchTerm=&MiscTypeId=
//   GET    /api/gateentry/MiscMaster/{id}       (returns 200 + data:null when not found — NO 404 guard)
//   GET    /api/gateentry/MiscMaster/by-name?term=&MiscTypeCode=
//
// Key facts that shaped assertions:
//   • code is alphanumeric (^[A-Za-z0-9]+$, max 20) — spaces/specials rejected. Immutable on update.
//   • description is REQUIRED on Create (NotEmpty, max 250).
//   • miscTypeId is a same-module FK (GateEntry MiscTypeMaster) validated via existence check.
//       → resolved at runtime via FirstIdAsync on /api/gateentry/MiscTypeMaster (fallback 1).
...
[Collection("GateMiscMasterCollection")]
[TestCaseOrderer("Shared.QAInfrastructure.Infrastructure.PriorityOrderer", "Shared.QAInfrastructure")]
public sealed class MiscMasterQATests
{
    private readonly QAServerFixture _f;
    private const string BaseRoute = "/api/gateentry/MiscMaster";
    private const string MiscTypeRoute = "/api/gateentry/MiscTypeMaster";

    private const string TestDescription = "QA Test Gate Misc Master";
    ...
    {
        // MiscType existence check false → FK validation fails.
        var resp = await _f.Client.PostAsJsonAsync(BaseRoute, new
        {
            miscTypeId = 999999,
            code = NewCode(),
    ...
    public async Task TC023_GetAll_FilterByMiscTypeId_Returns200()
    {
        var resp = await _f.Client.GetAsync($"{BaseRoute}?PageNumber=1&PageSize=15&MiscTypeId={_miscTypeId}");
    ...
    public async Task TC050_Update_HappyPath_Returns200()
    {
        ...
        {
            id = _f.CreatedId,
            description = "QA Updated Gate Misc Master",
            sortOrder = 1,
            isActive = 1
        });
