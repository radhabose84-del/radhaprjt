...
namespace GateEntryManagement.IntegrationTests.Repositories.VehicleMovementRecord
{
    ...
    public sealed class VehicleMovementRecordQueryRepositoryTests
    {
        ...
        // --- IsVehicleReceivingTypeAsync ---

        private async Task<(int vehicleId, int courierId)> SeedReceivingTypeMiscAsync(ApplicationDbContext ctx)
        {
            var typeRepo = new MiscTypeMasterCommandRepository(ctx);
            var rtTypeId = await typeRepo.CreateAsync(new GateEntryManagement.Domain.Entities.MiscTypeMaster
            {
            ...
            var miscRepo = new MiscMasterCommandRepository(ctx);
            var vehicleId = await miscRepo.CreateAsync(new GateEntryManagement.Domain.Entities.MiscMaster
            {
                MiscTypeId = rtTypeId,
                Code = "VEHICLE",
                Description = "Vehicle",
            ...
            var courierId = await miscRepo.CreateAsync(new GateEntryManagement.Domain.Entities.MiscMaster
            {
                MiscTypeId = rtTypeId,
                Code = "COURIER",
                Description = "Courier",
            ...
            return (vehicleId, courierId);
        }

        [Fact]
        public async Task IsVehicleReceivingTypeAsync_Should_Return_True_For_Vehicle()
        {
            await using var ctx = _fixture.CreateFreshDbContext();
            await SeedPrerequisitesAsync(ctx);
            var (vehicleId, _) = await SeedReceivingTypeMiscAsync(ctx);

            var result = await CreateQueryRepo().IsVehicleReceivingTypeAsync(vehicleId);

            result.Should().BeTrue();
        }

        [Fact]
        public async Task IsVehicleReceivingTypeAsync_Should_Return_False_For_Courier()
        {
            await using var ctx = _fixture.CreateFreshDbContext();
            await SeedPrerequisitesAsync(ctx);
            var (_, courierId) = await SeedReceivingTypeMiscAsync(ctx);

            var result = await CreateQueryRepo().IsVehicleReceivingTypeAsync(courierId);

            result.Should().BeFalse();
        }

        [Fact]
        public async Task IsVehicleReceivingTypeAsync_Should_Return_False_For_Unrelated_Misc()
        {
            await using var ctx = _fixture.CreateFreshDbContext();
            var (_, vmrStatusMiscId) = await SeedPrerequisitesAsync(ctx);
            await SeedReceivingTypeMiscAsync(ctx);

            // VMRStatus misc is not a ReceivingType -> false
            var result = await CreateQueryRepo().IsVehicleReceivingTypeAsync(vmrStatusMiscId);
