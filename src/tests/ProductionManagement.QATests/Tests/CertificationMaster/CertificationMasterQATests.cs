namespace ProductionManagement.QATests.Tests.CertificationMaster;

// ─────────────────────────────────────────────────────────────────────────────
// CertificationMaster (Production) — live-server QA suite (CRUD lifecycle + negatives).
//
// Contract verified against source (2026-06-17):
//   POST   /api/CertificationMaster            { certificationName, description? }
//   PUT    /api/CertificationMaster            { id, certificationName, description?, isActive }
//   DELETE /api/CertificationMaster?id={id}    (DeleteCertificationMaster(int id) → bound from QUERY)
//   GET    /api/CertificationMaster?PageNumber=&PageSize=&SearchTerm=
//   GET    /api/CertificationMaster/{id}       (returns 200 + data:null when not found — NO 404 guard)
//   GET    /api/CertificationMaster/by-name?term=
//
// Note: route has NO /production/ prefix (uses bare api/[controller]).
//
// Key facts that shaped assertions:
//   • CertificationName is alphanumeric (^[A-Za-z0-9]+$), max 50, unique. Mutable on update (sent in PUT).
//   • Description is OPTIONAL, max 255.
//   • GetById data carries the name under "certificationName".
//   • Delete validator: NotEmpty (id!=0) → NotFound (must exist).
// ─────────────────────────────────────────────────────────────────────────────

[Collection("CertificationMasterCollection")]
[TestCaseOrderer("Shared.QAInfrastructure.Infrastructure.PriorityOrderer", "Shared.QAInfrastructure")]
public sealed class CertificationMasterQATests
{
    private readonly QAServerFixture _f;
    private const string BaseRoute = "/api/CertificationMaster";

    // The run-unique alphanumeric name captured at create; reused by duplicate test.
    private static string _createdName = string.Empty;

    public CertificationMasterQATests(QAServerFixture fixture) => _f = fixture;
    ...
    public async Task TC001_Create_HappyPath_Returns200_And_CapturesId()
    {
        ...
        {
            certificationName = _createdName,
    ...
    public async Task TC004_Create_NameTooLong_Returns400()
    {
        var resp = await _f.Client.PostAsJsonAsync(BaseRoute, new
        {
            certificationName = new string('A', 101), // exceeds max 50
            description = "Desc"
        });
    ...
    [Fact, TestPriority(5)]
    public async Task TC005_Create_NameWithSpace_Returns400()
    {
        // Alphanumeric pattern ^[A-Za-z0-9]+$ — spaces are not allowed.
        var resp = await _f.Client.PostAsJsonAsync(BaseRoute, new
        {
            certificationName = "QA CERT",
            description = "Desc"
        });
    ...
    [Fact, TestPriority(6)]
    public async Task TC006_Create_NameWithSpecialChar_Returns400()
    {
        var resp = await _f.Client.PostAsJsonAsync(BaseRoute, new
        {
            certificationName = "QA@CERT",
    ...
    [Fact, TestPriority(7)]
    public async Task TC007_Create_DuplicateName_Returns400()
    {
    ...
    public async Task TC030_GetById_ValidId_Returns200_WithCorrectName()
    {
        ...
        await QAHelper.AssertOkAsync(resp);
        var doc = await QAHelper.ParseAsync(resp);
        doc.RootElement.GetProperty("data").GetProperty("certificationName").GetString()
    ...
    // ─────────────────────────────────────────────────────────────────────────
    // SECTION 5 — UPDATE  (certificationName is sent on PUT)
    // ─────────────────────────────────────────────────────────────────────────
    ...
    public async Task TC054_Update_NonExistentId_Returns400_NotFound()
    {
        ...
        {
            id = 999999,
            certificationName = "QAUpdatedCert",
            isActive = 1
        });
