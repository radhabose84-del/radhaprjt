namespace ProductionManagement.QATests.Tests.YarnTwistMaster;

// ─────────────────────────────────────────────────────────────────────────────
// YarnTwistMaster (Production) — live-server QA suite (CRUD lifecycle + negatives).
//
// Contract verified against source (2026-06-17):
//   POST   /api/YarnTwistMaster            { twistName, description? }
//   PUT    /api/YarnTwistMaster            { id, twistName, description?, isActive }
//   DELETE /api/YarnTwistMaster?id={id}    (DeleteYarnTwistMaster(int id) → bound from QUERY)
//   GET    /api/YarnTwistMaster?PageNumber=&PageSize=&SearchTerm=
//   GET    /api/YarnTwistMaster/{id}       (returns 200 + data:null when not found — NO 404 guard)
//   GET    /api/YarnTwistMaster/by-name?term=
//
// Route note: the controller uses [Route("api/[controller]")] → /api/YarnTwistMaster
//   (the entity name "YarnTwist" is spelled correctly; there is NO 'yarntwiest' misspelling
//    in the controller route token).
//
// Key facts that shaped assertions:
//   • TwistName is REQUIRED, max 50, unique. NO alphanumeric restriction — letters/spaces allowed.
//       → therefore NO alphanumeric negative test; only duplicate-name negative.
//   • Description is OPTIONAL, max 255.
//   • GetById data carries the name under "twistName".
//   • Delete validator: NotEmpty (id!=0) → NotFound (must exist).
// ─────────────────────────────────────────────────────────────────────────────

[Collection("YarnTwistMasterCollection")]
[TestCaseOrderer("Shared.QAInfrastructure.Infrastructure.PriorityOrderer", "Shared.QAInfrastructure")]
public sealed class YarnTwistMasterQATests
{
    private readonly QAServerFixture _f;
    private const string BaseRoute = "/api/YarnTwistMaster";

    // The run-unique name captured at create; reused by duplicate test. Letters allowed (no alphanum rule).
    private static string _createdName = string.Empty;

    public YarnTwistMasterQATests(QAServerFixture fixture) => _f = fixture;

    // Run-unique name (alphanumeric prefix is fine; spaces also allowed but kept simple), sliced to 10 chars.
    private string NewName() => _f.EntityCode[..10];
    ...
    public async Task TC004_Create_NameTooLong_Returns400()
    {
        var resp = await _f.Client.PostAsJsonAsync(BaseRoute, new
        {
            twistName = new string('A', 101), // exceeds max 50
            description = "Desc"
        });
    ...
    public async Task TC030_GetById_ValidId_Returns200_WithCorrectName()
    {
        ...
        await QAHelper.AssertOkAsync(resp);
        var doc = await QAHelper.ParseAsync(resp);
        doc.RootElement.GetProperty("data").GetProperty("twistName").GetString()
    ...
    {
        var resp = await _f.Client.GetAsync($"{BaseRoute}/by-name?term=Q");
    ...
    {
        var resp = await _f.AnonymousClient.GetAsync($"{BaseRoute}/by-name?term=Q");
        resp.StatusCode.Should().Be(HttpStatusCode.Unauthorized);
    }

    // ─────────────────────────────────────────────────────────────────────────
    // SECTION 5 — UPDATE  (twistName is sent on PUT)
    // ─────────────────────────────────────────────────────────────────────────
