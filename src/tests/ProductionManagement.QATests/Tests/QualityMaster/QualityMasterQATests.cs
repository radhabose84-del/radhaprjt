namespace ProductionManagement.QATests.Tests.QualityMaster;

// ─────────────────────────────────────────────────────────────────────────────
// QualityMaster (Production) — live-server QA suite (CRUD lifecycle + negatives).
//
// Contract verified against source (2026-06-17):
//   POST   /api/QualityMaster            { qualityName, description? }
//   PUT    /api/QualityMaster            { id, qualityName, description?, isActive }
//   DELETE /api/QualityMaster?id={id}    (DeleteQualityMaster(int id) → bound from QUERY)
//   GET    /api/QualityMaster?PageNumber=&PageSize=&SearchTerm=
//   GET    /api/QualityMaster/{id}       (returns 200 + data:null when not found — NO 404 guard)
//   GET    /api/QualityMaster/by-name?term=
//
// Key facts that shaped assertions:
//   • QualityName is alphanumeric (^[A-Za-z0-9]+$), max 50, unique. Mutable on update (sent in PUT).
//   • Description is OPTIONAL, max 255.
//   • GetById data carries the name under "qualityName".
//   • Delete validator: NotEmpty (id!=0) → NotFound (must exist).
// ─────────────────────────────────────────────────────────────────────────────

[Collection("QualityMasterCollection")]
[TestCaseOrderer("Shared.QAInfrastructure.Infrastructure.PriorityOrderer", "Shared.QAInfrastructure")]
public sealed class QualityMasterQATests
{
    private readonly QAServerFixture _f;
    private const string BaseRoute = "/api/QualityMaster";
    ...
    public QualityMasterQATests(QAServerFixture fixture) => _f = fixture;
    ...
    public async Task TC004_Create_NameTooLong_Returns400()
    {
        var resp = await _f.Client.PostAsJsonAsync(BaseRoute, new
        {
            qualityName = new string('A', 101), // exceeds max 50
            description = "Desc"
        });
    ...
    public async Task TC030_GetById_ValidId_Returns200_WithCorrectName()
    {
        ...
        await QAHelper.AssertOkAsync(resp);
        var doc = await QAHelper.ParseAsync(resp);
        doc.RootElement.GetProperty("data").GetProperty("qualityName").GetString()
    ...
    // ─────────────────────────────────────────────────────────────────────────
    // SECTION 5 — UPDATE  (qualityName is sent on PUT)
    // ─────────────────────────────────────────────────────────────────────────
