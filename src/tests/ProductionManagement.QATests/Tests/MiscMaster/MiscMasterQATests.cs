namespace ProductionManagement.QATests.Tests.MiscMaster;

// ─────────────────────────────────────────────────────────────────────────────
// MiscMaster (Production) — live-server QA suite (CRUD lifecycle + negatives).
//
// Contract verified against source (2026-06-17):
//   POST   /api/production/MiscMaster             { miscTypeId, code, description }  (all 3 REQUIRED)
//   PUT    /api/production/MiscMaster             { id, description, sortOrder, isActive } (code + miscTypeId immutable)
//   DELETE /api/production/MiscMaster?id={id}     (DeleteMiscMaster(int id) → bound from QUERY)
//   GET    /api/production/MiscMaster?PageNumber=&PageSize=&SearchTerm=&MiscTypeId=
//   GET    /api/production/MiscMaster/{id}        (returns 200 + data:null when not found — NO 404 guard)
//   GET    /api/production/MiscMaster/by-name?term=&miscTypeCode=
//
// Key facts that shaped assertions:
//   • Code is alphanumeric (^[A-Za-z0-9]+$), max 100. Immutable on update.
//   • Description is REQUIRED on Create (NotEmpty), max 200.
//   • MiscTypeId is a same-module FK (Production.MiscTypeMaster) validated at runtime.
//       → resolved via FirstIdAsync on /api/production/MiscTypeMaster (fallback 1).
...
[Collection("ProdMiscMasterCollection")]
[TestCaseOrderer("Shared.QAInfrastructure.Infrastructure.PriorityOrderer", "Shared.QAInfrastructure")]
public sealed class MiscMasterQATests
{
    private readonly QAServerFixture _f;
    private const string BaseRoute = "/api/production/MiscMaster";
    ...
    public async Task TC006_Create_CodeTooLong_Returns400()
    {
        ...
        {
            miscTypeId = await ResolveMiscTypeIdAsync(),
            code = new string('A', 201), // exceeds max 100
            description = TestDescription
        });
