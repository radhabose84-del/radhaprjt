namespace ProductionManagement.QATests.Tests.ProcessMaster;

// ─────────────────────────────────────────────────────────────────────────────
// ProcessMaster (Production) — live-server QA suite (CRUD lifecycle + negatives).
//
// Contract verified against source (2026-06-17):
//   POST   /api/ProcessMaster            { processName, combingRequired, description? }
//   PUT    /api/ProcessMaster            { id, processName, combingRequired, description?, isActive }
//   DELETE /api/ProcessMaster?id={id}    (DeleteProcessMaster(int id) → bound from QUERY)
//   GET    /api/ProcessMaster?PageNumber=&PageSize=&SearchTerm=
//   GET    /api/ProcessMaster/{id}       (returns 200 + data:null when not found — NO 404 guard)
//   GET    /api/ProcessMaster/by-name?term=
//
// Key facts that shaped assertions:
//   • ProcessName is alphanumeric (^[A-Za-z0-9]+$), max 50, unique. Mutable on update (sent in PUT).
//   • CombingRequired is a bool flag.
//   • Description is OPTIONAL, max 255.
//   • GetById data carries the name under "processName".
//   • Delete validator: NotEmpty (id!=0) → NotFound (must exist).
// ─────────────────────────────────────────────────────────────────────────────

[Collection("ProcessMasterCollection")]
[TestCaseOrderer("Shared.QAInfrastructure.Infrastructure.PriorityOrderer", "Shared.QAInfrastructure")]
public sealed class ProcessMasterQATests
{
    private readonly QAServerFixture _f;
    private const string BaseRoute = "/api/ProcessMaster";
    ...
    public ProcessMasterQATests(QAServerFixture fixture) => _f = fixture;
    ...
    public async Task TC004_Create_NameTooLong_Returns400()
    {
        var resp = await _f.Client.PostAsJsonAsync(BaseRoute, new
        {
            processName = new string('A', 101), // exceeds max 50
            combingRequired = false,
            description = "Desc"
        });
    ...
    public async Task TC030_GetById_ValidId_Returns200_WithCorrectName()
    {
        ...
        await QAHelper.AssertOkAsync(resp);
        var doc = await QAHelper.ParseAsync(resp);
        doc.RootElement.GetProperty("data").GetProperty("processName").GetString()
    ...
    // ─────────────────────────────────────────────────────────────────────────
    // SECTION 5 — UPDATE  (processName + combingRequired are sent on PUT)
    // ─────────────────────────────────────────────────────────────────────────
