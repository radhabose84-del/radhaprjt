namespace ProductionManagement.QATests.Tests.PackType;

// ─────────────────────────────────────────────────────────────────────────────
// PackType (Production) — live-server QA suite (CRUD lifecycle + negatives).
//
// Contract verified against source (2026-06-17):
//   POST   /api/PackType            { packTypeCode, packTypeName, netWeight, tareWeight?, conesPerBag?, packMaterialId? }
//   PUT    /api/PackType            { id, packTypeName, netWeight, tareWeight?, conesPerBag?, packMaterialId?, isActive }  (code immutable)
//   DELETE /api/PackType?id={id}    (DeletePackType(int id) → bound from QUERY)
//   GET    /api/PackType?PageNumber=&PageSize=&SearchTerm=
//   GET    /api/PackType/{id}       (returns 200 + data:null when not found — NO 404 guard)
//   GET    /api/PackType/by-name?term=
//
// Key facts that shaped assertions:
//   • PackTypeCode is alphanumeric (^[A-Za-z0-9]+$), max 20, unique. Immutable on update.
//   • PackTypeName is REQUIRED, max 100, unique.
//   • NetWeight is REQUIRED decimal and must be > 0.
//   • TareWeight (decimal, >= 0), ConesPerBag (int, >= 0), PackMaterialId (optional FK → MiscMaster).
//   • GetById data carries the code under "packTypeCode".
//   • Delete validator: NotEmpty (id!=0) → NotFound (must exist).
// ─────────────────────────────────────────────────────────────────────────────

[Collection("PackTypeCollection")]
[TestCaseOrderer("Shared.QAInfrastructure.Infrastructure.PriorityOrderer", "Shared.QAInfrastructure")]
public sealed class PackTypeQATests
{
    private readonly QAServerFixture _f;
    private const string BaseRoute = "/api/PackType";

    private const string TestName = "QA Test Pack Type";
    ...
    public PackTypeQATests(QAServerFixture fixture) => _f = fixture;
    ...
    public async Task TC001_Create_HappyPath_Returns200_And_CapturesId()
    {
        ...
        {
            packTypeCode = _createdCode,
            packTypeName = TestName + " " + _createdCode,
            netWeight = 25.5m,
            tareWeight = 0.5m,
            conesPerBag = 10
        });
    ...
    public async Task TC002_Create_NoAuthToken_Returns401()
    {
        ...
        {
            packTypeCode = "NOAUTH01",
            packTypeName = "No Auth Pack",
            netWeight = 10m
        });
    ...
    public async Task TC005_Create_CodeTooLong_Returns400()
    {
        var resp = await _f.Client.PostAsJsonAsync(BaseRoute, new
        {
            packTypeCode = new string('A', 51), // exceeds max 20
            packTypeName = TestName,
            netWeight = 10m
        });
    ...
    [Fact, TestPriority(8)]
    public async Task TC008_Create_NetWeightZero_Returns400()
    {
        // NetWeight must be > 0.
        var resp = await _f.Client.PostAsJsonAsync(BaseRoute, new
        {
    ...
    public async Task TC030_GetById_ValidId_Returns200_WithCorrectCode()
    {
        ...
        await QAHelper.AssertOkAsync(resp);
        var doc = await QAHelper.ParseAsync(resp);
        doc.RootElement.GetProperty("data").GetProperty("packTypeCode").GetString()
    ...
    public async Task TC050_Update_HappyPath_Returns200()
    {
        ...
        {
            id = _f.CreatedId,
            packTypeName = "QA Updated Pack " + _createdCode,
            netWeight = 30m,
            tareWeight = 1m,
            conesPerBag = 12,
            isActive = 1
    ...
    public async Task TC053_Update_NetWeightZero_Returns400()
    {
        var resp = await _f.Client.PutAsJsonAsync(BaseRoute, new
        {
            id = _f.CreatedId,
