namespace ProductionManagement.QATests.Tests.CountGroup;

// ─────────────────────────────────────────────────────────────────────────────
// CountGroup (Production) — live-server QA suite (CRUD lifecycle + negatives).
//
// Contract verified against source (2026-06-17):
//   POST   /api/CountGroup            { countGroupCode, countGroupName, description? }
//   PUT    /api/CountGroup            { id, countGroupName, description?, isActive }  (countGroupCode immutable)
//   DELETE /api/CountGroup?id={id}    (DeleteCountGroup(int id) → bound from QUERY)
//   GET    /api/CountGroup?PageNumber=&PageSize=&SearchTerm=
//   GET    /api/CountGroup/{id}       (returns 200 + data:null when not found — NO 404 guard)
//   GET    /api/CountGroup/by-name?term=
//
// Key facts that shaped assertions:
//   • CountGroupCode is alphanumeric (^[A-Za-z0-9]+$), max 50, unique. Immutable on update.
//   • CountGroupName is REQUIRED, max 100, unique.
//   • Description is OPTIONAL, max 200.
//   • GetById data carries the code under "countGroupCode".
//   • Delete validator: NotEmpty (id!=0) → NotFound (must exist).
// ─────────────────────────────────────────────────────────────────────────────

[Collection("CountGroupCollection")]
[TestCaseOrderer("Shared.QAInfrastructure.Infrastructure.PriorityOrderer", "Shared.QAInfrastructure")]
public sealed class CountGroupQATests
{
    private readonly QAServerFixture _f;
    private const string BaseRoute = "/api/CountGroup";

    private const string TestName = "QA Test Count Group";
    ...
    public CountGroupQATests(QAServerFixture fixture) => _f = fixture;
    ...
    public async Task TC001_Create_HappyPath_Returns200_And_CapturesId()
    {
        ...
        {
            countGroupCode = _createdCode,
            countGroupName = TestName + " " + _createdCode,
    ...
    public async Task TC002_Create_NoAuthToken_Returns401()
    {
        ...
        {
            countGroupCode = "NOAUTH01",
            countGroupName = "No Auth Group",
            description = "No auth"
        });
    ...
    public async Task TC005_Create_CodeTooLong_Returns400()
    {
        var resp = await _f.Client.PostAsJsonAsync(BaseRoute, new
        {
            countGroupCode = new string('A', 101), // exceeds max 50
            countGroupName = TestName,
            description = "Desc"
        });
    ...
    public async Task TC006_Create_NameTooLong_Returns400()
    {
        ...
        {
            countGroupCode = NewCode(),
            countGroupName = new string('A', 201), // exceeds max 100
            description = "Desc"
        });
    ...
    public async Task TC030_GetById_ValidId_Returns200_WithCorrectCode()
    {
        ...
        await QAHelper.AssertOkAsync(resp);
        var doc = await QAHelper.ParseAsync(resp);
        doc.RootElement.GetProperty("data").GetProperty("countGroupCode").GetString()
    ...
    public async Task TC050_Update_HappyPath_Returns200()
    {
        ...
        {
            id = _f.CreatedId,
            countGroupName = "QA Updated Count Group " + _createdCode,
    ...
    public async Task TC051_Update_NoAuthToken_Returns401()
    {
        ...
        {
            id = _f.CreatedId,
            countGroupName = "QA Updated Count Group",
            isActive = 1
        });
