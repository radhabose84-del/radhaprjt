...
namespace ProductionManagement.QATests.Tests.Security;

// ─────────────────────────────────────────────────────────────────────────────
// Phase 3 — Security: authentication-bypass checks (ProductionManagement surface).
//
...
// (documented live behavior, same middleware as SalesManagement/UserManagement).
...
public sealed class AuthBypassQATests
{
    ...
    private const string ProtectedGet  = "/api/qualitymaster?PageNumber=1&PageSize=5";
    private const string ProtectedPost = "/api/qualitymaster";
    ...
    // CONTROL — a genuine token IS accepted (tolerant 200/404; QualityMaster may be empty).
    [Fact, TestPriority(1)]
    ...
    public async Task TC007_Post_NoToken_Returns401()
    {
        ...
        {
            qualityCode = "SEC01",
            qualityName = "Sec Bypass Attempt"
        });
        resp.StatusCode.Should().Be(HttpStatusCode.Unauthorized);
    }
}
