...
public sealed class DeleteSalesReturnCommandHandlerTests
{
    ...
    private DeleteSalesReturnCommandHandler CreateSut() =>
    ...
    public async Task Handle_ExistingId_ReturnsTrue()
    {
        ...
        var result = await CreateSut().Handle(new DeleteSalesReturnCommand(1), CancellationToken.None);
    ...
    public async Task Handle_ExistingId_PublishesAuditEvent()
    {
        ...
        await CreateSut().Handle(new DeleteSalesReturnCommand(1), CancellationToken.None);

        _mockMediator.Verify(
            m => m.Publish(
                It.Is<AuditLogsDomainEvent>(e => e.ActionCode == "SALES_RETURN_DELETE"),
    ...
    public async Task Handle_NonExistentId_ThrowsExceptionRules()
    {
        ...
        Func<Task> act = async () =>
            await CreateSut().Handle(new DeleteSalesReturnCommand(99), CancellationToken.None);
    ...
    public async Task Handle_NonExistentId_DoesNotPublishAuditEvent()
    {
        ...
        try { await CreateSut().Handle(new DeleteSalesReturnCommand(99), CancellationToken.None); }
        catch { /* expected */ }
