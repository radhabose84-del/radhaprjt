...
{
    public class DeleteFreightMasterCommandHandlerTests
    {
        ...
        public async Task Handle_ValidId_PublishesAuditLogEvent_Once()
        {
            ...
            _mockMediator.Verify(
                m => m.Publish(
                    It.Is<AuditLogsDomainEvent>(e => e.ActionCode == "FREIGHT_MASTER_DELETE"),
        ...
        public async Task Handle_NonExistentId_DoesNotPublishAuditEvent()
        {
            ...
            var sut = CreateSut();
            try { await sut.Handle(new DeleteFreightMasterCommand(99), CancellationToken.None); }
            catch { /* expected */ }
