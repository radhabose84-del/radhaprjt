...
namespace SalesManagement.UnitTests.Application.SalesGroup.Commands
{
    public class DeleteSalesGroupCommandHandlerTests
    {
        ...
        public async Task Handle_ValidId_ReturnsTrue()
        {
            SetupSoftDelete(1);
            SetupPublishAudit();
            var result = await CreateSut().Handle(new DeleteSalesGroupCommand(1), CancellationToken.None);
        ...
        public async Task Handle_ValidId_CallsSoftDeleteAsync_Once()
        {
            SetupSoftDelete(1);
            SetupPublishAudit();
            await CreateSut().Handle(new DeleteSalesGroupCommand(1), CancellationToken.None);
