...
public class DeleteSalesContactCommandHandlerTests
{
    ...
    public async Task Handle_ValidId_ReturnsTrue()
    {
        ...
        var result = await CreateSut().Handle(new DeleteSalesContactCommand(1), CancellationToken.None);
    ...
    public async Task Handle_ValidId_CallsSoftDeleteAsync_Once()
    {
        ...
        await CreateSut().Handle(new DeleteSalesContactCommand(1), CancellationToken.None);
    ...
    public async Task Handle_ValidId_PublishesAuditLogEvent_Once()
    {
        ...
        _mockMediator.Verify(
            m => m.Publish(
                It.Is<AuditLogsDomainEvent>(e => e.ActionCode == "SALES_CONTACT_DELETE"),
    ...
    public async Task Handle_NonExistentId_ThrowsExceptionRules()
    {
        ...
        Func<Task> act = async () => await sut.Handle(
            new DeleteSalesContactCommand(99), CancellationToken.None);
