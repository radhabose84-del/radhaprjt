...
public sealed class DeleteSalesQuotationCommandHandlerTests
{
    private readonly Mock<ISalesQuotationCommandRepository> _mockCommandRepo = new(MockBehavior.Strict);
    private readonly Mock<IMediator> _mockMediator = new(MockBehavior.Loose);

    private DeleteSalesQuotationCommandHandler CreateSut() =>
    ...
    public async Task Handle_ExistingId_ReturnsTrue()
    {
        ...
        var result = await CreateSut().Handle(new DeleteSalesQuotationCommand(1), CancellationToken.None);
    ...
    public async Task Handle_NonExistentId_ThrowsExceptionRules()
    {
        ...
        Func<Task> act = async () =>
            await CreateSut().Handle(new DeleteSalesQuotationCommand(99), CancellationToken.None);
    ...
    public async Task Handle_ExistingId_PublishesAuditEvent()
    {
        ...
        await CreateSut().Handle(new DeleteSalesQuotationCommand(1), CancellationToken.None);

        _mockMediator.Verify(
            m => m.Publish(
                It.Is<AuditLogsDomainEvent>(e => e.ActionCode == "SALESQUOTATION_DELETE"),
