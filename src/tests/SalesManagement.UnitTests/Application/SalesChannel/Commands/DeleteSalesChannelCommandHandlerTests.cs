...
namespace SalesManagement.UnitTests.Application.SalesChannel.Commands
{
    public class DeleteSalesChannelCommandHandlerTests
    {
        ...
        public async Task Handle_ValidId_ReturnsTrue()
        {
            SetupSoftDelete(1);
            SetupPublishAudit();
            var result = await CreateSut().Handle(new DeleteSalesChannelCommand(1), CancellationToken.None);
        ...
        public async Task Handle_ValidId_CallsSoftDeleteAsync_Once()
        {
            SetupSoftDelete(1);
            SetupPublishAudit();
            await CreateSut().Handle(new DeleteSalesChannelCommand(1), CancellationToken.None);
            ...
            _mockMediator.Verify(
                m => m.Publish(
                    It.Is<AuditLogsDomainEvent>(e =>
                        ...
                        e.ActionCode == "SALES_CHANNEL_DELETE" &&
