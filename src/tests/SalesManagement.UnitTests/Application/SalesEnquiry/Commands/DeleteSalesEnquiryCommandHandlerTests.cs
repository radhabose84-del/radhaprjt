...
public sealed class DeleteSalesEnquiryCommandHandlerTests
{
    private readonly Mock<ISalesEnquiryCommandRepository> _mockCommandRepo = new(MockBehavior.Strict);
    private readonly Mock<IMediator> _mockMediator = new(MockBehavior.Loose);

    private DeleteSalesEnquiryCommandHandler CreateSut() =>
    ...
    public async Task Handle_ExistingId_ReturnsTrue()
    {
        ...
        var result = await CreateSut().Handle(new DeleteSalesEnquiryCommand(1), CancellationToken.None);
    ...
    public async Task Handle_ExistingId_PublishesAuditEvent()
    {
        ...
        await CreateSut().Handle(new DeleteSalesEnquiryCommand(1), CancellationToken.None);

        _mockMediator.Verify(
            m => m.Publish(
                It.Is<AuditLogsDomainEvent>(e =>
                    e.ActionDetail == "SoftDelete" &&
                    e.ActionCode == "SALESENQUIRY_DELETE"),
    ...
    public async Task Handle_NonExistentId_ThrowsExceptionRules()
    {
        ...
        Func<Task> act = async () =>
            await CreateSut().Handle(new DeleteSalesEnquiryCommand(99), CancellationToken.None);
    ...
    public async Task Handle_NonExistentId_DoesNotPublishAuditEvent()
    {
        ...
        try { await CreateSut().Handle(new DeleteSalesEnquiryCommand(99), CancellationToken.None); }
        catch { /* expected */ }
