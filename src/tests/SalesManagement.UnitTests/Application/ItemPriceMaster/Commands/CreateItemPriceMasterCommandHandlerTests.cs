    ...
    public class CreateItemPriceMasterCommandHandlerTests
    {
        ...
        private void SetupHappyPath(CreateItemPriceMasterCommand command, int newId = 1)
        {
            ...
            _mockItemLookup
                ...
                {
                    new ItemLookupDto { Id = command.ItemId, ItemCode = "ITEM001", ItemName = "LED Bulb" }
                });

            _mockQueryRepo
                .Setup(r => r.GetNextPriceCodeSerialAsync("LED"))
                .ReturnsAsync(1);
        ...
        [Fact]
        public async Task Handle_ValidCommand_AuditEvent_ContainsAutoGeneratedPriceCode()
        {
            var command = ItemPriceMasterBuilders.ValidCreateCommand();
            SetupHappyPath(command, newId: 1);
            ...
            _mockMediator.Verify(
                m => m.Publish(
                    It.Is<AuditLogsDomainEvent>(e => e.ActionName == "LED-001"),
                    It.IsAny<CancellationToken>()),
                Times.Once);
        }
        ...
        [Fact]
        public async Task Handle_ValidCommand_SetsPriceCodeOnEntity()
        {
            var command = ItemPriceMasterBuilders.ValidCreateCommand();
            ...
            capturedEntity!.PriceCode.Should().Be("LED-001");
        }
