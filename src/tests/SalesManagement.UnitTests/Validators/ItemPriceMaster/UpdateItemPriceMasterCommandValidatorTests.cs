...
namespace SalesManagement.UnitTests.Validators.ItemPriceMaster
{
    ...
    /// </summary>
    public class UpdateItemPriceMasterCommandValidatorTests
    {
        private readonly Mock<IItemPriceMasterQueryRepository> _mockQueryRepo = new(MockBehavior.Strict);

        private UpdateItemPriceMasterCommandValidator CreateValidator()
            => new UpdateItemPriceMasterCommandValidator(
        ...
        {
            SetupAllValid();
            var command = ItemPriceMasterBuilders.ValidUpdateCommand();
        ...
        {
            SetupAllValid();
            var command = ItemPriceMasterBuilders.ValidUpdateCommand(id: id);

            var result = await CreateValidator().TestValidateAsync(command);

            result.ShouldHaveValidationErrorFor(x => x.Id)
                  .WithErrorMessage("Valid Item Price Master Id is required.");
        }
        ...
        public async Task Id_NotFound_FailsValidation()
        {
            ...
            result.ShouldHaveValidationErrorFor(x => x.Id)
                  .WithErrorMessage("Item Price Master not found.");
        }
        ...
        {
            SetupAllValid();
            var command = ItemPriceMasterBuilders.ValidUpdateCommand(itemId: itemId);
        ...
        public async Task ItemId_NotFound_FailsValidation()
        {
            ...
            var command = ItemPriceMasterBuilders.ValidUpdateCommand(itemId: 10);
        ...
        {
            SetupAllValid();
            var command = ItemPriceMasterBuilders.ValidUpdateCommand(salesSegmentId: segmentId);
        ...
        public async Task SalesSegmentId_NotFound_FailsValidation()
        {
            ...
            var command = ItemPriceMasterBuilders.ValidUpdateCommand(salesSegmentId: 1);
        ...
        {
            SetupAllValid();
            var command = ItemPriceMasterBuilders.ValidUpdateCommand(exMillPrice: price);
        ...
        public async Task CurrencyId_NotFound_FailsValidation()
        {
            ...
            var command = ItemPriceMasterBuilders.ValidUpdateCommand(currencyId: 5);
        ...
        public async Task ValidTo_BeforeValidFrom_FailsValidation()
        {
            ...
            var command = ItemPriceMasterBuilders.ValidUpdateCommand(validFrom: validFrom, validTo: validTo);
        ...
        {
            SetupAllValid();
            var command = ItemPriceMasterBuilders.ValidUpdateCommand(isActive: isActive);
