...
{
    public sealed class DeleteSalesChannelCommandValidatorTests
    {
        private readonly Mock<ISalesChannelQueryRepository> _mockQueryRepo = new(MockBehavior.Strict);

        private DeleteSalesChannelCommandValidator CreateValidator()
        ...
        public async Task ValidId_ExistingEntity_PassesValidation()
        {
            ...
            var result = await CreateValidator().TestValidateAsync(new DeleteSalesChannelCommand(1));
        ...
        public async Task Id_Zero_FailsNotEmpty()
        {
            ...
            var result = await CreateValidator().TestValidateAsync(new DeleteSalesChannelCommand(0));
        ...
        public async Task Id_EntityNotFound_FailsValidation()
        {
            ...
            var result = await CreateValidator().TestValidateAsync(new DeleteSalesChannelCommand(999));

            result.ShouldHaveValidationErrorFor(x => x.Id)
                  .WithErrorMessage("Sales Channel not found.");
        }
        ...
        public async Task Id_ActiveRelationshipsExist_FailsSoftDeleteValidation()
        {
            ...
            var result = await CreateValidator().TestValidateAsync(new DeleteSalesChannelCommand(2));
