...
{
    public sealed class GetAllGatePassQueryHandlerTests
    {
        private readonly Mock<IGatePassQueryRepository> _mockQueryRepo = new(MockBehavior.Strict);
        private readonly Mock<IMapper> _mockMapper = new(MockBehavior.Loose);
        private readonly Mock<IMediator> _mockMediator = new(MockBehavior.Loose);

        private GetAllGatePassQueryHandler CreateSut() =>
            new(_mockQueryRepo.Object, _mockMapper.Object, _mockMediator.Object);

        [Fact]
        public async Task Handle_ReturnsSuccess()
        {
            var dtoList = new List<GatePassHdrDto>
            {
                new GatePassHdrDto { Id = 1, GatePassNo = "GP001", VehicleNumber = "KA01AB1234" }
            };
            _mockQueryRepo
            ...
            var result = await CreateSut().Handle(
                new GetAllGatePassQuery { PageNumber = 1, PageSize = 10 },
        ...
        public async Task Handle_ReturnsPaginationMetadata()
        {
            var dtoList = new List<GatePassHdrDto>
            {
                new GatePassHdrDto { Id = 1, GatePassNo = "GP001" }
            };
            _mockQueryRepo
            ...
            var result = await CreateSut().Handle(
                new GetAllGatePassQuery { PageNumber = 2, PageSize = 5, SearchTerm = "search" },
                CancellationToken.None);
        ...
        public async Task Handle_EmptyResult_ReturnsSuccess()
        {
            _mockQueryRepo
                .Setup(r => r.GetAllAsync(1, 10, null))
                .ReturnsAsync((new List<GatePassHdrDto>(), 0));
            ...
            _mockMediator.Verify(
                m => m.Publish(
                    It.Is<AuditLogsDomainEvent>(e =>
                        e.ActionDetail == "GetAllGatePassQuery" &&
                        e.ActionCode == "Get"),
