namespace PartyManagement.QATests.Tests.AuditLog;

// ─────────────────────────────────────────────────────────────────────────────
// AuditLog (Party) — live-server QA suite (READ-ONLY log report).
//
// Contract verified against source (2026-06-17 — AuditLogController.cs):
//   ⚠ Route prefix is "api/party/[controller]" (NOT "api/AuditLog"):
//   GET /api/party/AuditLog                                  (list all — no params, raw Ok(auditLogs))
//   GET /api/party/AuditLog/GetAuditLogSearch?searchPattern= (search by free-text pattern)
//
...
[Collection("PartyAuditLogCollection")]
[TestCaseOrderer("Shared.QAInfrastructure.Infrastructure.PriorityOrderer", "Shared.QAInfrastructure")]
public sealed class AuditLogQATests
{
    private readonly QAServerFixture _f;
    private const string BaseRoute = "/api/party/AuditLog";
