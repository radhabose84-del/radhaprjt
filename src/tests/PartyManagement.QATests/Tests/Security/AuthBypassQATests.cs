...
namespace PartyManagement.QATests.Tests.Security;

// ─────────────────────────────────────────────────────────────────────────────
// Phase 3 — Security: authentication-bypass checks (PartyManagement surface).
//
...
public sealed class AuthBypassQATests
{
    ...
    private const string ProtectedGet  = "/api/bankmaster?pageNumber=1&pageSize=5";
    private const string ProtectedPost = "/api/bankmaster";
    ...
    public async Task TC007_Post_NoToken_Returns401()
    {
        ...
        {
            dto = new { bankName = "SecBypassBank" }
        });
        resp.StatusCode.Should().Be(HttpStatusCode.Unauthorized);
    }
}
