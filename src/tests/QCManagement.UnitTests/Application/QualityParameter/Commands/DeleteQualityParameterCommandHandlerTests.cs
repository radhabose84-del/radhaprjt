...
namespace QCManagement.UnitTests.Application.QualityParameter.Commands
{
    public class DeleteQualityParameterCommandHandlerTests
    {
        ...
        public async Task Handle_ValidId_ReturnsTrue()
        {
            SetupSoftDelete(1);
            SetupPublishAudit();
            var result = await CreateSut().Handle(new DeleteQualityParameterCommand(1), CancellationToken.None);
        ...
        public async Task Handle_ValidId_CallsSoftDeleteAsync_Once()
        {
            SetupSoftDelete(1);
            SetupPublishAudit();
            await CreateSut().Handle(new DeleteQualityParameterCommand(1), CancellationToken.None);
            ...
            _mockMediator.Verify(
                m => m.Publish(
                    It.Is<AuditLogsDomainEvent>(e =>
                        ...
                        e.ActionCode == "QUALITY_PARAMETER_DELETE"),
