...
namespace QCManagement.QATests.Tests.Security;

// ─────────────────────────────────────────────────────────────────────────────
// Phase 3 — Security: authentication-bypass checks (QCManagement surface).
//
...
public sealed class AuthBypassQATests
{
    ...
    private const string ProtectedGet  = "/api/qc/miscmaster?PageNumber=1&PageSize=5";
    private const string ProtectedPost = "/api/qc/miscmaster";
    ...
    // CONTROL — a genuine token IS accepted (tolerant 200/404: list may be empty on the clone).
    [Fact, TestPriority(1)]
