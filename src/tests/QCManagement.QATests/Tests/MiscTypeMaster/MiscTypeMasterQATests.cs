namespace QCManagement.QATests.Tests.MiscTypeMaster;

// ─────────────────────────────────────────────────────────────────────────────
// QC MiscTypeMaster — live-server QA suite (CRUD lifecycle + negatives).
//
// Contract verified against source (2026-06-16):
//   POST   /api/qc/MiscTypeMaster          { miscTypeCode, description }   (description REQUIRED)
//   PUT    /api/qc/MiscTypeMaster          { id, description, isActive }   (code immutable)
//   DELETE /api/qc/MiscTypeMaster?id={id}  (id bound from QUERY, not route)
//   GET    /api/qc/MiscTypeMaster?PageNumber=&PageSize=&SearchTerm=
//   GET    /api/qc/MiscTypeMaster/{id}     (returns 200 + data:null when not found — NO 404 guard)
//   GET    /api/qc/MiscTypeMaster/by-name?term=
//
// Key facts that shaped assertions:
//   • MiscTypeCode is alphanumeric (^[A-Za-z0-9]+$) — spaces/specials rejected. Immutable on update.
//   • Description is REQUIRED on Create (NotEmpty) — TC004 asserts the empty-description 400.
//   • MaxLength sourced from EF metadata (code 20, description 250) → "longer than" message.
//   • No FK. AlreadyExists checks code uniqueness (IsDeleted=0).
//   • Delete validator: NotEmpty (id!=0) → NotFound (must exist).
// ─────────────────────────────────────────────────────────────────────────────

[Collection("QcMiscTypeMasterCollection")]
[TestCaseOrderer("Shared.QAInfrastructure.Infrastructure.PriorityOrderer", "Shared.QAInfrastructure")]
public sealed class MiscTypeMasterQATests
{
    private readonly QAServerFixture _f;
    private const string BaseRoute = "/api/qc/MiscTypeMaster";

    private const string TestDescription = "QA Test QC Misc Type";
    ...
    {
        var letters = new string(_f.EntityCode
            .Select(c => char.IsDigit(c) ? (char)('A' + (c - '0')) : c)
            .Where(char.IsLetter)
            .ToArray());
        return letters[..Math.Min(10, letters.Length)];
    }
    ...
    public async Task TC005_Create_CodeTooLong_Returns400()
    {
        var resp = await _f.Client.PostAsJsonAsync(BaseRoute, new
        {
            miscTypeCode = new string('A', 101), // exceeds code max (20)
            description = TestDescription
        });
    ...
    {
        if (_f.CreatedId == 0) return; // create blocked → nothing to find
    ...
    public async Task TC050_Update_HappyPath_Returns200()
    {
        ...
        {
            id = _f.CreatedId,
            description = "QA Updated QC Misc Type",
            isActive = 1
        });
