namespace LogisticsManagement.QATests.Tests.MiscTypeMaster;

// ─────────────────────────────────────────────────────────────────────────────
// LogisticsManagement.MiscTypeMaster — live-server QA suite (CRUD lifecycle + negatives).
//
// Contract verified against source (2026-06-16):
//   POST   /api/MiscTypeMaster            { miscTypeCode, description }  (both REQUIRED)
//   PUT    /api/MiscTypeMaster            { id, description, isActive }  (miscTypeCode immutable)
//   DELETE /api/MiscTypeMaster?id={id}    ([HttpDelete] int id → id bound from QUERY, not route)
//   GET    /api/MiscTypeMaster?PageNumber=&PageSize=&SearchTerm=
//   GET    /api/MiscTypeMaster/{id}       (returns 200 + data:null when not found — NO 404 guard)
//   GET    /api/MiscTypeMaster/by-name?term=
//
// Key facts that shaped assertions:
//   • Route has NO /logistics/ prefix — controller is [Route("api/[controller]")].
//   • Code (miscTypeCode) is alphanumeric (^[A-Za-z0-9]+$) — spaces/specials rejected. Immutable on update.
//   • Description is REQUIRED on Create (NotEmpty).
//   • AlreadyExists is on miscTypeCode (unique) — duplicate test reuses the captured code.
//   • GetById data carries the code under "miscTypeCode".
//   • Delete validator: NotEmpty (id!=0) → NotFound (must exist).
// ─────────────────────────────────────────────────────────────────────────────

[Collection("LogisticsMiscTypeMasterCollection")]
[TestCaseOrderer("Shared.QAInfrastructure.Infrastructure.PriorityOrderer", "Shared.QAInfrastructure")]
public sealed class MiscTypeMasterQATests
{
    private readonly QAServerFixture _f;
    private const string BaseRoute = "/api/MiscTypeMaster";
