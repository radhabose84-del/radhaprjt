...
namespace FinanceManagement.UnitTests.Application.AccountGroup.Queries
{
    public sealed class GetAccountGroupLeavesQueryHandlerTests
    {
        ...
        public async Task Handle_ReturnsLeafGroups()
        {
            ...
            var result = await CreateSut().Handle(new GetAccountGroupLeavesQuery(1, null), CancellationToken.None);

            result.Should().ContainSingle();
        }

        [Fact]
        public async Task Handle_PassesCompanyAndAccountTypeToRepository()
        {
            _mockQueryRepo
                .Setup(r => r.GetLeafGroupsAsync(7, 3))
                .ReturnsAsync(new List<AccountGroupLookupDto>());

            await CreateSut().Handle(new GetAccountGroupLeavesQuery(7, 3), CancellationToken.None);
        ...
        public async Task Handle_PublishesAuditEvent()
        {
            ...
            await CreateSut().Handle(new GetAccountGroupLeavesQuery(1, null), CancellationToken.None);
