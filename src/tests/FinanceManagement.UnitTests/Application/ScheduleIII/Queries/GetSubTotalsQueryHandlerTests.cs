...
namespace FinanceManagement.UnitTests.Application.ScheduleIII.Queries
{
    public sealed class GetSubTotalsQueryHandlerTests
    {
        ...
        public async Task Handle_ReturnsSuccessWithData()
        {
            ...
            _mockQueryRepo.Setup(r => r.GetSubTotalsAsync(1)).ReturnsAsync(data);

            var result = await CreateSut().Handle(new GetSubTotalsQuery { StructureId = 1 }, CancellationToken.None);
        ...
        {
            _mockQueryRepo.Setup(r => r.GetSubTotalsAsync(1)).ReturnsAsync(new List<ScheduleIIISubTotalDto>());
